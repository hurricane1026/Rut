...
- Bitwise ops are functions in the built-in `bitwise` namespace, not symbols: `bitwise.and`/`bitwise.or`/`bitwise.xor`/`bitwise.flip`/`bitwise.shiftLeft`/`bitwise.shiftRight` — implemented end-to-end (i32 and i64, same-width operands with bare-literal adoption; shift amounts share the operand width and saturate outside 0..width-1; `flip(a)` desugars to `xor(a, -1)`; literal operands fold at analyze time; the removed `&`/`~`/`<<` symbols emit a fix-it)
- Arithmetic `+ - * / %` and unary minus — implemented end-to-end (i32 and i64, same-width operands; two's-complement wrap on overflow; `x / 0` and `x % 0` evaluate to 0; `INT_MIN / -1` → `INT_MIN`, `INT_MIN % -1` → 0, same at 64-bit; literal `/ 0` is a compile error; literal operands fold at analyze time)
- i64: int literals that don't fit i32 auto-widen to i64 (`60000000000` works; beyond i64 → error); `i64(x)` widens an i32 value (Swift-initializer style, shadowed by a user func named `i64`); mixed non-literal widths are a compile error with an `i64(x)` fix-it (bare literals adopt the i64 side); i64 is unnameable in type position (no annotations/struct fields/params); `match` on i64 is ⏳ rejected for now
- `time.nowMicros() -> i64` — monotonic microseconds, **latched per handler invocation** (every use of a now-bound local sees the same value; fresh across requests); rejected in routes containing `wait` (locals re-materialize on resume); `max(a, b)`/`min(a, b)` — same-width {i32,i64} builtins, single evaluation, literal folding; all three shadowed by user bindings of the same name
- `match` with `=>` (expression) or `{}` (block); no `case` keyword. `if` always uses `{}`
- Header access is function-style: `req.header("X-Request-ID")`, `req.set(...)`, `resp.set(...)`, `resp.remove(...)` — hyphenated property access does not exist (would parse as subtraction). Request set/add and Response-builder set/add/remove/header are wired; Response header names stay literal, while values may be runtime strings on direct routes without guards/decorators/wait/for. Buffered/post-middleware Response mutation remains pending.
- `Cache<IP, i64>` state (DESIGN.md §3.3.6): top-level `let buckets = Cache<IP, i64>(capacity: N)`; `buckets.get(key) -> i64?` (miss = never-seen OR evicted — always handle it, blessed idiom `.or(0)`), `buckets.set(key, v)` (bare statement allowed only before guards/for; routes containing wait reject every cache op because writes run at handler entry); per-shard lossy 4-way slot table, seeded hashing; deliberately NOT named Hash (lossless prior)
...
- **Error handling**: Result + `guard let` / `if let` + `.or(default)` + `x == nil`/`x != nil` presence test (nil and error uniformly "absent"; never-nil sources are a compile error; guard let / if let work over pure-optional carriers too); no postfix `x?`, no `?.` / `??` / force-unwrap `!`; no exceptions
