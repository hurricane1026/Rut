...
- `Cache<IP, i64>` state (docs/state-types.md): top-level `let buckets = Cache<IP, i64>(capacity: N)`; `buckets.get(key) -> i64?` (miss = never-seen OR evicted — always handle it, blessed idiom `.or(0)`), `buckets.set(key, v)` (bare statement allowed ONLY before guards/waits — state writes run at handler entry); per-shard lossy 4-way slot table, seeded hashing; deliberately NOT named Hash (lossless prior)
