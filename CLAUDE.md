...
- `time.nowMicros() -> i64` — monotonic microseconds, **latched per handler invocation** (every use of a now-bound local sees the same value; fresh across requests); `max(a, b)`/`min(a, b)` — same-width {i32,i64} builtins, single evaluation, literal folding; all three shadowed by user bindings of the same name
