...
> described below. Implementations must use its coordinator admission,
> indivisible per-shard installation, stable allocation identities, retained
> predecessor pins, and two-adjacent-generation bound. `reload()` means
> asynchronous admission (`true` is accepted, not activated); this historical
> section must not be used to infer runtime behavior.
