...
TEST(timer, periodic_memory_metrics_refresh_requires_a_consumer) {
    using namespace rut;
    RouteConfig idle_cfg{};
    const RouteConfig* active = &idle_cfg;
    auto loop = std::make_unique<EpollEventLoop>();
    loop->config_ptr = &active;
    ShardMetrics local;
    local.init();
    ...
    local.memory_arena_used = 123;
    loop->fire_due_timers();
    CHECK_EQ(local.memory_arena_used, 123u);

    loop->metrics_endpoint_enabled = true;
    loop->fire_due_timers();
    CHECK_EQ(local.memory_arena_used, 0u);

    RouteConfig snapshot_cfg{};
    REQUIRE(snapshot_cfg.add_jit_handler(
        "/stats", kRouteMethodGet, &timer_snapshot_probe_fn, false, false, true));
    CHECK(snapshot_cfg.has_control_plane_snapshot_consumer);
    active = &snapshot_cfg;
    loop->metrics_endpoint_enabled = false;
    local.memory_arena_used = 456;
    loop->fire_due_timers();
    CHECK_EQ(local.memory_arena_used, 0u);
}
