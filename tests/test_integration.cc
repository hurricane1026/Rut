...
i32 scripted_ssl_pending(SSL* /*ssl*/) {
    return g_scripted_tls ? g_scripted_tls->pending : 0;
}
...
static void run_upstream_tls_send_want_write_while_recv_paused_case(
    rut::test::TestCase* test_case) {
    auto* _tc = test_case;
    i32 fds[2];
    REQUIRE_EQ(socketpair(AF_UNIX, SOCK_STREAM | SOCK_NONBLOCK, 0, fds), 0);
    EpollBackend backend;
    REQUIRE(backend.init(0, -1).has_value());
    backend.upstream_fd_map[0] = fds[0];

    TestConn tc;
    tc.init(0, fds[0]);
    Connection& conn = tc.conn;
    conn.upstream_fd = fds[0];
    conn.upstream_tls = reinterpret_cast<SSL*>(0x1);
    conn.ws_client_send_pending = true;

    ScriptedTlsState tls_state;
    tls_state.write_first_rc = -1;
    tls_state.write_first_error = SSL_ERROR_WANT_WRITE;
    tls_state.write_second_rc = -1;
    tls_state.write_second_error = SSL_ERROR_WANT_WRITE;
    ScopedTlsHooks hooks(tls_state);

    static const u8 kPayload[] = {'p', 'i', 'n', 'g'};
    REQUIRE(backend.add_send_upstream_tls(conn, kPayload, sizeof(kPayload)));
    backend.pause_upstream_recv(0, true);

    IoEvent events[8];
    CHECK_EQ(backend.wait(events, 8, &conn, 1), 0u);
    CHECK_EQ(tls_state.write_calls, 2);
...
struct TlsOriginServer {
    i32 listen_fd = -1;
    u16 port = 0;
    TlsServerContext* tls = nullptr;
    std::atomic<bool> running{false};
    bool started = false;
    pthread_t thread{};

    ~TlsOriginServer() { teardown(); }

    static void* run(void* arg) {
        auto* self = static_cast<TlsOriginServer*>(arg);
        i32 client = -1;
        while (self->running.load(std::memory_order_acquire)) {
            client = accept(self->listen_fd, nullptr, nullptr);
            if (client >= 0) break;
        ...
        if (client >= 0) {
            ...
            auto ssl_result = create_tls_server_ssl(self->tls, client);
            if (ssl_result) {
                ...
                if (SSL_accept(ssl) == 1) {
                    ...
                    if (SSL_read(ssl, req, sizeof(req)) > 0) {
                        static constexpr char kResponse[] =
                            "HTTP/1.1 200 OK\r\nContent-Length: 6\r\nConnection: "
                            "close\r\n\r\nsecure";
                        (void)ssl_write_all(ssl, kResponse, sizeof(kResponse) - 1);
                    }
                }
...
TEST(timer, control_plane_snapshot_is_latched_before_timer_handler) {
    using namespace rut;
    RouteConfig cfg{};
    REQUIRE(cfg.add_timer("t",
                          1,
                          /*interval_ms=*/1,
                          &timer_snapshot_probe_fn,
                          /*shard=*/-1,
...
TEST(tls_state_machine, upstream_send_want_write_keeps_recv_paused) {
    run_upstream_tls_send_want_write_while_recv_paused_case(_tc);
}

TEST(tls_state_machine, upstream_tls_pending_plaintext_queues_read_completion) {
    i32 fds[2];
    REQUIRE_EQ(socketpair(AF_UNIX, SOCK_STREAM | SOCK_NONBLOCK, 0, fds), 0);
    ...
    conn.upstream_fd = fds[0];
    conn.upstream_tls = reinterpret_cast<SSL*>(0x1);
    conn.upstream_recv_buf.bind(tc.recv_storage, kTestBufSize);
    backend.upstream_send_state[0] = {
        nullptr, fds[0], 0, 1, IoEventType::UpstreamSend, true, EPOLLOUT};

    ScriptedTlsState tls_state;
    tls_state.pending = 1;
    tls_state.read_first_rc = 3;
    tls_state.read_first_error = SSL_ERROR_NONE;
    ScopedTlsHooks hooks(tls_state);

    REQUIRE(backend.add_recv_upstream_tls(conn));
    REQUIRE_EQ(backend.pending_count, 1u);
    IoEvent events[2];
    REQUIRE_EQ(backend.wait(events, 2, &conn, 1), 1u);
    CHECK_EQ(events[0].type, IoEventType::UpstreamRecv);
    CHECK_EQ(events[0].result, 3);
    CHECK_EQ(conn.upstream_recv_buf.len(), 3u);
...
TEST(tls_state_machine, upstream_tls_buffered_plaintext_queues_read_completion) {
    i32 fds[2];
    REQUIRE_EQ(socketpair(AF_UNIX, SOCK_STREAM | SOCK_NONBLOCK, 0, fds), 0);
    ...
    conn.upstream_tls = reinterpret_cast<SSL*>(0x1);
    conn.upstream_recv_buf.bind(tc.recv_storage, kTestBufSize);
    static constexpr u8 kResponse[] = {'o', 'k'};
    REQUIRE(conn.upstream_recv_buf.write(kResponse, sizeof(kResponse)));
    conn.pending_handler_fn = [](void*, jit::HandlerCtx*, const u8*, u32, void*) -> u64 {
        return 0;
    };
    ...
    REQUIRE_EQ(backend.wait(events, 2, &conn, 1), 1u);
    CHECK_EQ(events[0].type, IoEventType::UpstreamRecv);
    CHECK_EQ(events[0].result, static_cast<i32>(sizeof(kResponse)));
    CHECK_EQ(tls_state.read_calls, 0);
...
TEST(tls_state_machine, upstream_tls_pending_read_error_queues_failure) {
    i32 fds[2];
    REQUIRE_EQ(socketpair(AF_UNIX, SOCK_STREAM | SOCK_NONBLOCK, 0, fds), 0);
    EpollBackend backend;
    REQUIRE(backend.init(0, -1).has_value());
    ...
    tls_state.pending = 1;
    tls_state.read_first_rc = -1;
    tls_state.read_first_error = SSL_ERROR_SSL;
    ScopedTlsHooks hooks(tls_state);
    REQUIRE(backend.add_recv_upstream_tls(conn));
    IoEvent events[2];
    REQUIRE_EQ(backend.wait(events, 2, &conn, 1), 1u);
...
TEST(tls_state_machine, upstream_tls_recv_arms_socket_without_pending_plaintext) {
    i32 fds[2];
    REQUIRE_EQ(socketpair(AF_UNIX, SOCK_STREAM | SOCK_NONBLOCK, 0, fds), 0);
    EpollBackend backend;
    REQUIRE(backend.init(0, -1).has_value());
    ...
    ScriptedTlsState tls_state;
    ScopedTlsHooks hooks(tls_state);
    CHECK(backend.add_recv_upstream_tls(conn));
    CHECK_EQ(backend.upstream_fd_map[0], fds[0]);
...
TEST(tls_state_machine, upstream_tls_buffered_read_reports_full_receive_buffer) {
    i32 fds[2];
    REQUIRE_EQ(socketpair(AF_UNIX, SOCK_STREAM | SOCK_NONBLOCK, 0, fds), 0);
    EpollBackend backend;
    REQUIRE(backend.init(0, -1).has_value());
    ...
    conn.upstream_tls = reinterpret_cast<SSL*>(0x1);
    conn.upstream_recv_buf.bind(tc.recv_storage, kTestBufSize);
    conn.upstream_recv_buf.commit(kTestBufSize);
    conn.pending_handler_fn = [](void*, jit::HandlerCtx*, const u8*, u32, void*) -> u64 {
        return 0;
    };
    conn.pending_yield_kind = jit::YieldKind::UpstreamRecv;
    ...
    REQUIRE_EQ(backend.wait(events, 2, &conn, 1), 1u);
    CHECK_EQ(events[0].type, IoEventType::UpstreamRecv);
    CHECK_EQ(events[0].result, static_cast<i32>(kTestBufSize));
    CHECK_EQ(tls_state.read_calls, 0);
...
TEST(tls_state_machine, upstream_tls_recv_reclaims_delivered_buffer_before_reading_more) {
    i32 fds[2];
    REQUIRE_EQ(socketpair(AF_UNIX, SOCK_STREAM | SOCK_NONBLOCK, 0, fds), 0);
    EpollBackend backend;
    REQUIRE(backend.init(0, -1).has_value());
    ...
    conn.upstream_recv_buf.bind(tc.recv_storage, kTestBufSize);
    conn.upstream_recv_buf.commit(kTestBufSize);
    conn.upstream_jit_recv_delivered = kTestBufSize;
    ...
    CHECK_EQ(events[0].result, 3);
    CHECK_EQ(conn.upstream_recv_buf.len(), 3u);
    CHECK_EQ(conn.upstream_jit_recv_delivered, 0u);
    CHECK_EQ(tls_state.read_calls, 1);
...
TEST(tls_state_machine, send_rejects_out_of_range_conn_id_with_full_pending_ring) {
    ...
    REQUIRE(backend.add_send_tls(conn, kPayload, sizeof(kPayload)));
    CHECK_EQ(tls_state.write_calls, 1);
    REQUIRE_EQ(backend.pending_count, EpollBackend::kPendingCap);
    CHECK_EQ(backend.pending_completions[EpollBackend::kPendingCap - 1].conn_id,
             EpollBackend::kPendingCap - 1);
    CHECK_EQ(backend.pending_completions[EpollBackend::kPendingCap - 1].type, IoEventType::Recv);
...
TEST(shard, proxies_to_verified_tls_upstream) {
    TlsOriginServer origin;
    REQUIRE(origin.setup());
    auto tls_client = create_tls_client_context(kTestCertPath);
...
static u64 wait_upstream_connect_result_handler(
    ...
    if (ctx->state == 0) {
        return rut::jit::HandlerResult::make_yield_payload(
                   1, rut::jit::YieldKind::UpstreamConnect, 1)
            .pack();
    }
    return rut::jit::HandlerResult::make_status(ctx->resume_event_result == 0 ? 204 : 502).pack();
}

TEST(shard, jit_upstream_connect_wait_reports_tls_verification_failure) {
    TlsOriginServer origin;
    REQUIRE(origin.setup());
    auto tls_client = create_tls_client_context(kTestCertPath);
    REQUIRE(tls_client.has_value());
    ...
    auto upstream = config.add_upstream("secure", 0x7f000001u, origin.port);
    REQUIRE(upstream.has_value());
    REQUIRE(config.set_upstream_tls(upstream.value(), "wrong.example", 13));
    REQUIRE(config.add_jit_handler("/connect", 'G', &wait_upstream_connect_result_handler));
    ...
    REQUIRE(send_all(client, kRequest, sizeof(kRequest) - 1));
    char response[4096];
    const i32 n = recv_timeout(client, response, sizeof(response), 3000);
    REQUIRE(n > 0);
    CHECK(n >= 12 && memcmp(response, "HTTP/1.1 502", 12) == 0);
    close(client);
...
TEST(shard, rejects_tls_upstream_hostname_mismatch) {
    TlsOriginServer origin;
    REQUIRE(origin.setup());
    auto tls_client = create_tls_client_context(kTestCertPath);
    REQUIRE(tls_client.has_value());
...
TEST(tls, client_context_loads_system_ca_bundle_when_unspecified) {
    auto tls_ctx = create_tls_client_context(nullptr);
    REQUIRE(tls_ctx.has_value());
    destroy_tls_client_context(tls_ctx.value());
}
...
TEST(tls, server_ssl_rejects_missing_context) {
    CHECK(!create_tls_server_ssl(nullptr, 0).has_value());
}

TEST(tls, client_ssl_validates_inputs_and_initializes_connection) {
    auto tls_ctx = create_tls_client_context(kTestCertPath);
    REQUIRE(tls_ctx.has_value());

    CHECK(!create_tls_client_ssl(nullptr, 0, "localhost").has_value());
    CHECK(!create_tls_client_ssl(tls_ctx.value(), -1, "localhost").has_value());
    CHECK(!create_tls_client_ssl(tls_ctx.value(), 0, nullptr).has_value());
    CHECK(!create_tls_client_ssl(tls_ctx.value(), 0, "").has_value());

    const i32 fd = dup(2);
    REQUIRE(fd >= 0);
    auto ssl = create_tls_client_ssl(tls_ctx.value(), fd, "localhost");
    REQUIRE(ssl.has_value());
    destroy_tls_client_ssl(ssl.value());
    close(fd);
    destroy_tls_client_context(tls_ctx.value());
}

TEST(tls, server_ssl_initializes_from_verified_context) {
    auto tls_ctx = create_tls_server_context(kTestCertPath, kTestKeyPath);
    REQUIRE(tls_ctx.has_value());
    const i32 fd = dup(2);
    REQUIRE(fd >= 0);
    auto ssl = create_tls_server_ssl(tls_ctx.value(), fd);
    REQUIRE(ssl.has_value());
    destroy_tls_server_ssl(ssl.value());
    close(fd);
    destroy_tls_server_context(tls_ctx.value());
}
...
// A request-backed response header recorded before wait() points into the
// synthesized request scratch. Both its HandlerCtx and that view must survive
// until the timer resumes the stream.
TEST(shard, serves_http2_wait_header_value_survives_suspend) {
    using namespace rut;
...
route GET "/slow" {
    ...
    REQUIRE(engine.init());
    REQUIRE(engine.compile(cg.mod, cg.ctx));
    auto slow = reinterpret_cast<jit::HandlerFn>(engine.lookup("handler_route_0"));
    REQUIRE(slow != nullptr);

    RouteConfig cfg{};
    REQUIRE(cfg.add_jit_handler("/slow", 'G', slow));
    const RouteConfig* active = &cfg;
    ...
    u8 out[768];
    u32 n = h2_client_prologue(out);
    n += h2_client_get(out + n, sizeof(out) - n, 1, "/slow", 5);
    REQUIRE(write_all_fd(c, out, n));
    ...
    CHECK_EQ(h2_status_for_stream(resp, total, 1), 200u);
    CHECK(h2_response_has_header(resp, total, 1, "x-path", "/slow"));
...
static constexpr const char kBufferedForwardExpressionSource[] = R"rut(
upstream api
route GET "/capture" {
...
static constexpr const char kBufferedForwardRequestResumeSource[] = R"rut(
upstream api
route GET "/capture" {
    let resp = forward(api, buffered: true)
    resp.set("X-Path", req.path)
...
static constexpr const char kBufferedForwardDiscardSource[] = R"rut(
upstream api
route GET "/capture" {
    let resp = forward(api, buffered: true)
    return 404
...
static constexpr const char kBufferedHeadHeaderlessBodyMutationSource[] = R"rut(
upstream api
route HEAD "/capture" {
    let resp = forward(api, buffered: true)
    resp.body = "mutated"
...
static constexpr const char kBufferedBodylessReturnSource[] = R"rut(
upstream api
route GET "/capture" {
    let resp = forward(api, buffered: true)
    resp.status = 204
...
TEST(shard, control_plane_snapshot_is_latched_over_http1) {
    ...
    RouteConfig cfg;
    REQUIRE(cfg.add_jit_handler("/stats",
                                'G',
                                compiled.handler,
...
TEST(shard, buffered_forward_expression_owns_fields_across_yield_over_http1) {
    ScriptedUpstreamServer backend;
    static const char kResp[] =
    ...
    REQUIRE(backend.setup(kResp, sizeof(kResp) - 1));
    CompiledRutRoute compiled;
    REQUIRE(compiled.compile(kBufferedForwardExpressionSource));
    ...
    CHECK(buf_contains(response, total, "HTTP/1.1 207", 12));
...
TEST(shard, buffered_forward_expression_owns_fields_across_yield_over_http2) {
    ScriptedUpstreamServer backend;
    static const char kResp[] =
        "HTTP/1.1 206 Partial Content\r\n"
        "Content-Length: 8\r\n"
    ...
    u32 request_len = h2_client_prologue(request);
    request_len +=
        h2_client_get(request + request_len, sizeof(request) - request_len, 1, "/capture", 8);
    REQUIRE(write_all_fd(client, request, request_len));
    ...
    for (u32 attempt = 0; attempt < 10 && total < sizeof(response); attempt++) {
        ...
        total += static_cast<u32>(got);
        body_len = h2_body_for_stream(response, total, 1, body, sizeof(body));
        if (h2_status_for_stream(response, total, 1) == 207 && body_len == 8) break;
    }
    CHECK_EQ(h2_status_for_stream(response, total, 1), 207u);
    CHECK_EQ(body_len, 8u);
...
TEST(shard, buffered_forward_expression_preserves_http1_request_for_resume) {
    ScriptedUpstreamServer backend;
    static const char kResp[] =
        "HTTP/1.1 200 OK\r\nContent-Length: 2\r\nConnection: close\r\n\r\nok";
    REQUIRE(backend.setup(kResp, sizeof(kResp) - 1));
    CompiledRutRoute compiled;
    REQUIRE(compiled.compile(kBufferedForwardRequestResumeSource));
    RouteConfig cfg;
    REQUIRE(cfg.add_upstream("api", 0x7F000001, backend.port).has_value());
    REQUIRE(cfg.add_jit_handler("/capture", 'G', compiled.handler));
    Shard<EpollEventLoop> shard;
...
TEST(shard, captured_http1_head_suppresses_mutated_body_without_headers) {
    ...
    response[got] = '\0';
    CHECK(buf_contains(response, static_cast<u32>(got), "HTTP/1.1 200", 12));
    CHECK(buf_contains(response, static_cast<u32>(got), "Content-Length: 7\r\n", 19));
    const char* end = ::strstr(response, "\r\n\r\n");
    REQUIRE(end != nullptr);
    CHECK_EQ(static_cast<u32>(end + 4 - response), static_cast<u32>(got));
    close(client);
...
TEST(shard, discarded_buffered_capture_does_not_override_http1_literal_response) {
    ...
    CHECK(buf_contains(response, total, "HTTP/1.1 404", 12));
    CHECK(buf_contains(response, total, "Not Found", 9));
    CHECK_FALSE(buf_contains(response, total, "X-Origin", 8));
    CHECK_FALSE(buf_contains(response, total, "original", 8));
    close(client);
    shard.stop();
    shard.join();
    shard.shutdown();
...
TEST(shard, discarded_buffered_capture_does_not_override_http2_literal_response) {
    ...
    for (u32 attempt = 0; attempt < 10 && total < sizeof(response); attempt++) {
        ...
        if (got <= 0) break;
        total += static_cast<u32>(got);
        if (h2_status_for_stream(response, total, 1) == 404) break;
    }
    CHECK_EQ(h2_status_for_stream(response, total, 1), 404u);
    CHECK_FALSE(h2_response_has_header(response, total, 1, "x-origin", "leaked"));
    close(client);
    shard.stop();
    shard.join();
    shard.shutdown();
...
TEST(shard, captured_http2_bodyless_status_suppresses_data) {
    ScriptedUpstreamServer backend;
    static const char kResp[] =
        "HTTP/1.1 200 OK\r\nContent-Length: 8\r\nConnection: close\r\n\r\noriginal";
    REQUIRE(backend.setup(kResp, sizeof(kResp) - 1));
    CompiledRutRoute compiled;
    REQUIRE(compiled.compile(kBufferedBodylessReturnSource));
    RouteConfig cfg;
    REQUIRE(cfg.add_upstream("api", 0x7F000001, backend.port).has_value());
    REQUIRE(cfg.add_jit_handler("/capture", 'G', compiled.handler));
    Shard<EpollEventLoop> shard;
    ...
    for (u32 attempt = 0; attempt < 10 && total < sizeof(response); attempt++) {
        ...
        if (got <= 0) break;
        total += static_cast<u32>(got);
        if (h2_status_for_stream(response, total, 1) == 204) break;
    }
    u8 body[16];
    CHECK_EQ(h2_status_for_stream(response, total, 1), 204u);
    CHECK_EQ(h2_body_for_stream(response, total, 1, body, sizeof(body)), 0u);
    close(client);
    shard.stop();
    shard.join();
...
TEST(shard, oversized_captured_http2_headers_fail_before_resume) {
    ScriptedUpstreamServer backend;
    std::string upstream = "HTTP/1.1 200 OK\r\nX-Large: ";
    upstream.append(9000, 'x');
    upstream.append("\r\nContent-Length: 0\r\nConnection: close\r\n\r\n");
    REQUIRE(backend.setup(upstream.data(), static_cast<u32>(upstream.size())));
    CompiledRutRoute compiled;
    REQUIRE(compiled.compile(kBufferedForwardReturnSource));
    RouteConfig cfg;
    REQUIRE(cfg.add_upstream("api", 0x7F000001, backend.port).has_value());
    REQUIRE(cfg.add_jit_handler("/capture", 'G', compiled.handler));
    Shard<EpollEventLoop> shard;
    ...
    for (u32 attempt = 0; attempt < 10 && total < sizeof(response); attempt++) {
        ...
        if (got <= 0) break;
        total += static_cast<u32>(got);
        if (h2_status_for_stream(response, total, 1) == 502) break;
    }
    CHECK_EQ(h2_status_for_stream(response, total, 1), 502u);
    close(client);
    shard.stop();
    shard.join();
    shard.shutdown();
...
TEST(shard, buffered_capture_large_body_resumes_over_http2) {
    ...
    u8 body[12000];
    u32 body_len = 0;
    for (u32 attempt = 0; attempt < 20 && total < sizeof(response); attempt++) {
        const i32 got = recv_timeout(
            client, reinterpret_cast<char*>(response + total), sizeof(response) - total, 2000);
        if (got <= 0) break;
        total += static_cast<u32>(got);
    ...
    CHECK_EQ(h2_status_for_stream(response, total, 1), 200u);
    CHECK_EQ(body_len, 12000u);
    CHECK(body_len == 12000 && body[0] == 'x' && body[11999] == 'x');
    close(client);
    shard.stop();
    shard.join();
    shard.shutdown();
...
TEST(shard, buffered_capture_rejects_http1_upgrade_response) {
    ScriptedUpstreamServer backend;
    static const char kResp[] =
    ...
    CHECK(buf_contains(response, total, "HTTP/1.1 502", 12));
    close(client);
    shard.stop();
    shard.join();
    shard.shutdown();
...
TEST(shard, buffered_forward_expression_overflow_fails_closed_as_502) {
    ScriptedUpstreamServer backend;
    std::string oversized = "HTTP/1.1 200 OK\r\nContent-Length: 17000\r\nConnection: close\r\n\r\n";
    oversized.append(17000, 'x');
    REQUIRE(backend.setup(oversized.data(), static_cast<u32>(oversized.size())));
    CompiledRutRoute compiled;
    REQUIRE(compiled.compile(kBufferedForwardExpressionSource));
    ...
    CHECK(buf_contains(response, total, "HTTP/1.1 502", 12));
    CHECK(buf_contains(response, total, "Bad Gateway", 11));
...
TEST(shard, buffered_forward_expression_upstream_failure_fails_closed_as_502) {
    i32 dead_lfd = create_listen_socket(0).value_or(-1);
    REQUIRE(dead_lfd >= 0);
    const u16 dead_port = get_port(dead_lfd);
    close(dead_lfd);

    CompiledRutRoute compiled;
    REQUIRE(compiled.compile(kBufferedForwardExpressionSource));
    RouteConfig cfg;
    REQUIRE(cfg.add_upstream("api", 0x7F000001, dead_port).has_value());
    REQUIRE(cfg.add_jit_handler("/capture", 'G', compiled.handler));
    Shard<EpollEventLoop> shard;
    i32 lfd = create_listen_socket(0).value_or(-1);
    REQUIRE(lfd >= 0);
...
TEST(shard, serves_http2_proxy_to_verified_tls_upstream) {
    TlsOriginServer origin;
    REQUIRE(origin.setup());
    auto tls_client = create_tls_client_context(kTestCertPath);
    REQUIRE(tls_client.has_value());
    ...
    RouteConfig cfg;
    auto id = cfg.add_upstream("secure", 0x7F000001, origin.port);
    REQUIRE(id.has_value());
    REQUIRE(cfg.set_upstream_tls(id.value(), "localhost", 9));
    REQUIRE(cfg.add_proxy("/secure", 0, id.value()));
    ...
    u32 request_len = h2_client_prologue(request);
    request_len +=
        h2_client_get(request + request_len, sizeof(request) - request_len, 1, "/secure", 7);
    REQUIRE(write_all_fd(client, request, request_len));
    ...
    u32 total = 0;
    u32 body_len = 0;
    for (i32 attempt = 0; attempt < 10 && total < sizeof(response); attempt++) {
        const i32 got = recv_timeout(
            client, reinterpret_cast<char*>(response + total), sizeof(response) - total, 2000);
        if (got <= 0) break;
        total += static_cast<u32>(got);
        body_len = h2_body_for_stream(response, total, 1, body, sizeof(body));
        if (h2_status_for_stream(response, total, 1) != 0 && body_len == 6) break;
    }
    CHECK_EQ(h2_status_for_stream(response, total, 1), 200u);
    CHECK_EQ(body_len, 6u);
    CHECK(body_len == 6 && memcmp(body, "secure", 6) == 0);
...
TEST(route, populate_route_config_carries_verified_upstream_tls) {
    using namespace rut;
    const char* src =
        "upstream backend { host: \"127.0.0.1\", port: 8443, "
    ...
    FrontendRirModule rir{};
    REQUIRE(lower_to_rir(*mir_owned, rir));
    REQUIRE(rir.module.upstreams[0].tls_enabled);
    CHECK(rir.module.upstreams[0].tls_server_name.eq({"localhost", 9}));

    RouteConfig cfg{};
    REQUIRE(populate_route_config(cfg, rir.module));
    REQUIRE_EQ(cfg.upstream_count, 1u);
    CHECK(cfg.has_tls_upstreams());
    CHECK(cfg.upstreams[0].tls_enabled);
    CHECK_EQ(cfg.upstreams[0].tls_server_name_len, 9u);
    CHECK_EQ(strcmp(cfg.upstreams[0].tls_server_name, "localhost"), 0);
    rir.destroy();
}

TEST(route, upstream_tls_rejects_invalid_name_and_health_probe_combination) {
    using namespace rut;
    RouteConfig cfg{};
    auto uid = cfg.add_upstream("backend", 0x7f000001u, 8443);
    REQUIRE(uid.has_value());
    CHECK(!cfg.set_upstream_tls(uid.value(), nullptr, 9));
    CHECK(!cfg.set_upstream_tls(uid.value(), "", 0));
    REQUIRE(cfg.set_upstream_health_check(uid.value(), "/health", 7, 1000, 200));
    CHECK(!cfg.set_upstream_tls(uid.value(), "localhost", 9));
    RouteConfig reverse{};
    auto reverse_uid = reverse.add_upstream("backend", 0x7f000001u, 8443);
    REQUIRE(reverse_uid.has_value());
    REQUIRE(reverse.set_upstream_tls(reverse_uid.value(), "localhost", 9));
    CHECK(!reverse.set_upstream_tls(reverse_uid.value(), "bad\nname", 8));
    CHECK(!reverse.set_upstream_tls(reverse_uid.value(), "127.0.0.1", 9));
    CHECK_EQ(reverse.upstreams[reverse_uid.value()].tls_server_name_len, 9u);
    CHECK_EQ(strcmp(reverse.upstreams[reverse_uid.value()].tls_server_name, "localhost"), 0);
    CHECK(!reverse.set_upstream_health_check(reverse_uid.value(), "/health", 7, 1000, 200));

    RouteConfig invalid_dns{};
    auto invalid_uid = invalid_dns.add_upstream("backend", 0x7f000001u, 8443);
    REQUIRE(invalid_uid.has_value());
    CHECK(!invalid_dns.set_upstream_tls(invalid_uid.value(), "api.example.com:443", 19));
    CHECK(!invalid_dns.set_upstream_tls(invalid_uid.value(), "*.example.com", 13));
    CHECK(!invalid_dns.set_upstream_tls(invalid_uid.value(), "api/example", 11));
    CHECK(!invalid_dns.set_upstream_tls(invalid_uid.value(), "api..example", 12));
    CHECK(!invalid_dns.set_upstream_tls(invalid_uid.value(), "-api.example", 12));
    CHECK(!invalid_dns.set_upstream_tls(invalid_uid.value(), "api-.example", 12));
    REQUIRE(invalid_dns.set_upstream_tls(invalid_uid.value(), "api.example.com", 15));
}
...
TEST(route, populate_route_config_fingerprints_upstream_marking_policy) {
    ...
    const char* array_carrier =
        "func duplicate(values: [bool]) -> [[bool]] => [values, values, values, values]\n"
    ...
    const char* struct_carrier =
        "struct Verdict { healthy: bool, a: bool, b: bool, c: bool }\n"
        "upstream users at \"127.0.0.1:8080\"\n"
        "timer check_health, every: 5s, shard: 0 { for server in users.servers { "
        "users.mark(server, healthy: Verdict(healthy: true, a: false, b: true, c: false).healthy) "
        "} return 200 }\n";
...
TEST(route, marking_policy_accepts_arena_owned_variadic_constructors) {
    ...
    auto bool_type_result = builder.make_type(rir::TypeKind::Bool);
    REQUIRE(bool_type_result);
    const rir::Type* bool_type = bool_type_result.value();
    rir::FieldDef fields[4] = {
        {Str{"a", 1}, bool_type},
        {Str{"b", 1}, bool_type},
        {Str{"c", 1}, bool_type},
        {Str{"d", 1}, bool_type},
    };
    auto struct_result = builder.create_struct(Str{"Verdict", 7}, fields, 4);
    REQUIRE(struct_result);
    auto function_result = builder.create_function(Str{"check", 5}, Str{}, 0);
    ...
    auto block_result = builder.create_block(fn, Str{"entry", 5});
    REQUIRE(block_result);
    builder.set_insert_point(fn, block_result.value());
    rir::ValueId values[4]{};
    for (u32 i = 0; i < 4; i++) {
        auto value = builder.emit_const_bool(i % 2 == 0);
        REQUIRE(value);
        values[i] = value.value();
    }
    auto array = builder.emit_array_create(bool_type, values, 4);
    REQUIRE(array);
    auto record = builder.emit_struct_create(struct_result.value(), values, 4);
    REQUIRE(record);
    auto server = builder.emit_const_i64(rir::encode_server_token(0, 0));
    REQUIRE(server);
    auto mark = builder.emit_upstream_mark(0, server.value(), values[0]);
    REQUIRE(mark);
    ...
    CHECK_GT(fn->blocks[0].insts[5].operand_count, rir::kMaxInlineOperands);
    const u32 capacity = array_inst.extra_operand_capacity;
    array_inst.extra_operand_capacity = 0;
    RouteConfig undersized{};
    CHECK_FALSE(populate_route_config(undersized, rir.module));
    array_inst.extra_operand_capacity = capacity;
    rir::ValueId forged_extra[1]{};
    rir::ValueId* owned_extra = array_inst.extra_operands;
    array_inst.extra_operands = forged_extra;
    RouteConfig unowned{};
    CHECK_FALSE(populate_route_config(unowned, rir.module));
    array_inst.extra_operands = owned_extra;
    RouteConfig cfg{};
    CHECK(populate_route_config(cfg, rir.module));
    rir.destroy();
}
...
TEST(route, marking_policy_json_array_matches_codegen_type_support) {
    using namespace rut;
    const rir::Type bool_type{rir::TypeKind::Bool, nullptr, nullptr};
    const rir::Type i32_type{rir::TypeKind::I32, nullptr, nullptr};
    const rir::Type i64_type{rir::TypeKind::I64, nullptr, nullptr};
    ...
    const rir::Type duration_type{rir::TypeKind::Duration, nullptr, nullptr};
    const rir::Type optional_i64_type{rir::TypeKind::Optional, &i64_type, nullptr};
    rir::Type array_types[33]{};
    array_types[0] = {rir::TypeKind::Array, &i64_type, nullptr};
    for (u32 i = 1; i < 33; i++)
        array_types[i] = {rir::TypeKind::Array, &array_types[i - 1], nullptr};
    const rir::Type duration_array{rir::TypeKind::Array, &duration_type, nullptr};
    const rir::Type optional_array{rir::TypeKind::Array, &optional_i64_type, nullptr};
    ...
    const rir::Type struct_type{rir::TypeKind::Struct, nullptr, struct_def};
    const rir::Type struct_array{rir::TypeKind::Array, &struct_type, nullptr};
    rir::Value value{&array_types[31], {}, 0};
    rir::Function fn{};
    fn.values = &value;
    fn.value_count = 1;
    rir::Instruction append{};
    append.op = rir::Opcode::JsonAppendArray;
    append.result = rir::kNoValue;
    append.operand_count = 1;
    append.operands[0] = {0};

    CHECK(marking_policy_instruction_types_valid(fn, append));
    value.type = &array_types[32];
    CHECK_FALSE(marking_policy_instruction_types_valid(fn, append));
    value.type = &duration_array;
    CHECK_FALSE(marking_policy_instruction_types_valid(fn, append));
    value.type = &optional_array;
    CHECK_FALSE(marking_policy_instruction_types_valid(fn, append));
    CHECK(marking_policy_json_type_valid(&bool_type));
    CHECK(marking_policy_json_type_valid(&i32_type));
    CHECK(marking_policy_json_type_valid(&str_list_type));
    value.type = &struct_array;
    CHECK(marking_policy_instruction_types_valid(fn, append));
    struct_def->fields()[1].type = &duration_type;
    CHECK_FALSE(marking_policy_instruction_types_valid(fn, append));
}

TEST(route, marking_policy_accepts_typed_response_and_json_capture_operations) {
    using namespace rut;
    const rir::Type i32_type{rir::TypeKind::I32, nullptr, nullptr};
    const rir::Type str_type{rir::TypeKind::Str, nullptr, nullptr};
    const rir::Type optional_str_type{rir::TypeKind::Optional, &str_type, nullptr};
    rir::Value values[3] = {{&i32_type, {}, 0}, {&str_type, {}, 0}, {&optional_str_type, {}, 0}};
    rir::Function fn{};
    fn.values = values;
    fn.value_count = 3;
    rir::Instruction inst{};

    inst.op = rir::Opcode::JsonCapture;
    inst.result = {1};
    CHECK(marking_policy_instruction_types_valid(fn, inst));

    inst.operand_count = 1;
    inst.operands[0] = {2};
    inst.op = rir::Opcode::RespHeader;
    inst.result = {2};
    CHECK(marking_policy_instruction_types_valid(fn, inst));
    inst.operands[0] = {1};
    CHECK_FALSE(marking_policy_instruction_types_valid(fn, inst));
    ...
    inst.operands[0] = {0};
    inst.op = rir::Opcode::RespStatus;
    inst.result = {0};
    CHECK(marking_policy_instruction_types_valid(fn, inst));
    inst.op = rir::Opcode::RespSetStatus;
    inst.result = rir::kNoValue;
    CHECK(marking_policy_instruction_types_valid(fn, inst));

    inst.operands[0] = {1};
    inst.op = rir::Opcode::RespBody;
    inst.result = {1};
    CHECK(marking_policy_instruction_types_valid(fn, inst));
    inst.result = rir::kNoValue;
    for (const auto op :
         {rir::Opcode::RespSetHeader, rir::Opcode::RespAddHeader, rir::Opcode::RespSetBody}) {
        inst.op = op;
        CHECK(marking_policy_instruction_types_valid(fn, inst));
    }
}
...
TEST(route, marking_policy_fingerprint_normalizes_nested_struct_projection) {
    ...
    alignas(rir::StructDef) u8 middle_storage[sizeof(rir::StructDef) + sizeof(rir::FieldDef)]{};
    auto* middle_def = reinterpret_cast<rir::StructDef*>(middle_storage);
    middle_def->name = Str{"Middle", 6};
    middle_def->field_count = 1;
    middle_def->field_capacity = 1;
    middle_def->fields()[0] = rir::FieldDef{Str{"inner", 5}, &inner_type};
    const rir::Type middle_type{rir::TypeKind::Struct, nullptr, middle_def};
    ...
    outer_def->fields()[0] = rir::FieldDef{Str{"middle", 6}, &middle_type};
    ...
    policy[2].operand_count = 1;
    policy[2].operands[0] = {1};
    policy[2].imm.struct_ref.type = &middle_type;
    policy[3].op = rir::Opcode::StructCreate;
    policy[3].operand_count = 1;
    policy[3].operands[0] = {2};
    policy[3].imm.struct_ref.type = &outer_type;
    policy[4].op = rir::Opcode::OptWrap;
    policy[4].operand_count = 1;
    policy[4].operands[0] = {3};
    policy[5].op = rir::Opcode::OptUnwrap;
    policy[5].operand_count = 1;
    policy[5].operands[0] = {4};
    policy[6].op = rir::Opcode::ArrayCreate;
    policy[6].operand_count = 1;
    policy[6].operands[0] = {5};
    policy[7].op = rir::Opcode::ArrayCreate;
    policy[7].operand_count = 1;
    policy[7].operands[0] = {6};
    policy[8].op = rir::Opcode::CtxLoadSlotI32;
    policy[9].op = rir::Opcode::ArrayGet;
    policy[9].operand_count = 2;
    policy[9].operands[0] = {7};
    policy[9].operands[1] = {8};
    policy[10].op = rir::Opcode::CtxLoadSlotI32;
    policy[11].op = rir::Opcode::ArrayGet;
    policy[11].operand_count = 2;
    policy[11].operands[0] = {9};
    policy[11].operands[1] = {10};
    policy[12].op = rir::Opcode::StructField;
    policy[12].operand_count = 1;
    policy[12].operands[0] = {11};
    policy[12].imm.struct_ref.name = Str{"middle", 6};
    policy[12].imm.struct_ref.type = &middle_type;
    policy[13].op = rir::Opcode::StructField;
    policy[13].operand_count = 1;
    policy[13].operands[0] = {12};
    policy[13].imm.struct_ref.name = Str{"inner", 5};
    policy[13].imm.struct_ref.type = &inner_type;
    policy[14].op = rir::Opcode::StructField;
    policy[14].operand_count = 1;
    policy[14].operands[0] = {13};
    policy[14].imm.struct_ref.name = Str{"server", 6};
    policy[14].imm.struct_ref.type = &i64_type;
    policy[15].op = rir::Opcode::ConstBool;
    policy[15].imm.bool_val = true;
    policy[16].op = rir::Opcode::UpstreamMark;
...
TEST(route, marking_policy_fingerprint_normalizes_dynamic_array_server_sources) {
    ...
    policy[1].op = rir::Opcode::ConstI64;
    policy[1].result = {1};
    policy[1].imm.i64_val = rir::encode_server_token(0, 1);
...
TEST(route, marking_policy_traces_arrays_through_struct_and_optional_carriers) {
    using namespace rut;
    const rir::Type i32_type{rir::TypeKind::I32, nullptr, nullptr};
    const rir::Type i64_type{rir::TypeKind::I64, nullptr, nullptr};
    const rir::Type bool_type{rir::TypeKind::Bool, nullptr, nullptr};
    ...
    const auto check_carrier = [&](bool use_struct) {
        rir::Value values[8] = {
            {&i64_type, {0}, 0},
            {&array_type, {0}, 1},
            {use_struct ? &struct_type : &optional_array_type, {0}, 2},
            {&array_type, {0}, 3},
            {&i32_type, {0}, 4},
            {&i64_type, {0}, 5},
            {&bool_type, {0}, 6},
        ...
        policy[1].operand_count = 1;
        policy[1].operands[0] = {0};
        policy[2].op = use_struct ? rir::Opcode::StructCreate : rir::Opcode::OptWrap;
        policy[2].result = {2};
        policy[2].operand_count = 1;
        policy[2].operands[0] = {1};
        policy[2].imm.struct_ref.type = use_struct ? &struct_type : nullptr;
        policy[3].op = use_struct ? rir::Opcode::StructField : rir::Opcode::OptUnwrap;
        policy[3].result = {3};
        policy[3].operand_count = 1;
        policy[3].operands[0] = {2};
        policy[3].imm.struct_ref.name = use_struct ? Str{"values", 6} : Str{};
        policy[3].imm.struct_ref.type = use_struct ? &array_type : nullptr;
        policy[4].op = rir::Opcode::ConstI32;
        policy[4].result = {4};
        policy[4].imm.i32_val = 0;
        policy[5].op = rir::Opcode::ArrayGet;
        policy[5].result = {5};
        policy[5].operand_count = 2;
        policy[5].operands[0] = {3};
        policy[5].operands[1] = {4};
        policy[6].op = rir::Opcode::ConstBool;
        policy[6].result = {6};
        policy[6].imm.bool_val = true;
        policy[7].op = rir::Opcode::UpstreamMark;
        policy[7].result = {7};
        policy[7].operand_count = 2;
        policy[7].operands[0] = {5};
        policy[7].operands[1] = {6};
        rir::Block block{{0}, {}, policy, 8, 8};
        rir::Function fn{};
        fn.blocks = &block;
        fn.block_count = 1;
        fn.block_cap = 1;
        ...
        fn.value_count = 8;
        fn.value_cap = 8;
        return marking_policy_value_is_mark_server(fn, {0}) &&
               marking_policy_array_flows_to_source(fn, {3}, {0}, 0);
    };

    CHECK(check_carrier(true));
    CHECK(check_carrier(false));
}
...
TEST(route, register_jit_routes_selects_segment_trie_and_adds_param_route) {
    ...
    CHECK_FALSE(cfg.routes[0].needs_control_plane_snapshot);
