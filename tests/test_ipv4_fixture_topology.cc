...
// Process-start monotonic reference for the phase progress lines below. A
// CTest timeout sends SIGKILL, which drops any output buffered but not yet
...
class PhaseScope {
public:
    explicit PhaseScope(const char* name) : name_(name) { log_phase(name_, "begin"); }
    ~PhaseScope() { log_phase(name_, "end"); }
...
int main() {
    ...
    // Declared here (rather than where it is first used, in the
    // sidecar_normal phase below) because run_with_held_topology_and_sidecar
    // is also called from the sidecar_failure_matrix, holder_suppressed_removal,
    // and sidecar_revalidation_matrix phases, each of which is its own nested
    // scope for PhaseScope timing.
    ...
    {
        PhaseScope phase_self_checks("self_checks");
    ...
    {
        PhaseScope phase_probe_policy_checks("probe_policy_checks");
        ...
        if (!rut::test::ipv4_topology::validate_held_topology_probe_evidence(
                ...
                validation_error)) {
            std::cerr << "FAIL [#358 Stage 2a3b socketless topology probe policy]: "
                      << validation_error << "\n";
            return 1;
        }
    ...
    {
        PhaseScope phase_preflight("preflight");
        const RunResult preflight =
            rut::test::ipv4_topology::run(FailurePoint::AfterNetworkACreated);
    ...
    {
        PhaseScope phase_failure_injection_matrix("failure_injection_matrix");
    ...
    {
        PhaseScope phase_full_run("full_run");
    ...
    {
        PhaseScope phase_held_topology("held_topology");
        bool held_callback_ran = false;
        const RunResult held = rut::test::ipv4_topology::run_with_held_topology(
            [&](const rut::test::ipv4_topology::HeldTopologySnapshot& snapshot,
                std::string& error) {
                ...
                return require(rut::test::ipv4_topology::validate_held_topology_probe_evidence(
                       ...
                       require(snapshot.network_a_name != snapshot.network_b_name,
                               "network_names") &&
    ...
    {
        PhaseScope phase_sidecar_normal("sidecar_normal");
        ...
        if (sidecar_normal.prerequisite_failure || !sidecar_normal.success ||
            ...
            std::cerr << "FAIL [#358 held-namespace sidecar normal lifecycle]: "
                      << sidecar_normal.error << "\n";
            return 1;
        }
    }
    {
        PhaseScope phase_sidecar_failure_matrix("sidecar_failure_matrix");
        for (const auto& injection :
             {std::pair{
                  HeldNamespaceSidecarFailurePoint::CreateSuppressedNoObject,
              ...
              std::pair{
                  ...
                  std::string("verified unexpected sidecar death: exact stopped identity and no "
                              "live /proc witness")},
              std::pair{
                  HeldNamespaceSidecarFailurePoint::DisappearBeforeCleanup,
                  std::string("verified already-disappeared sidecar settlement and safe topology "
                              "cleanup")},
              std::pair{
                  HeldNamespaceSidecarFailurePoint::PauseAfterSidecarSettlement,
    ...
    {
        PhaseScope phase_holder_suppressed_removal("holder_suppressed_removal");
    ...
    {
        PhaseScope phase_recreation_matrix("recreation_matrix");
        ...
        for (const RecreationCase& recreation :
                ...
                rut::test::ipv4_topology::run_with_recreated_sidecar(
                    [&](const rut::test::ipv4_topology::RecreatedSidecarEvidence& evidence,
                        ...
                        if (evidence.complete_generation ||
                            ...
                            (stopped &&
                             evidence.state != RecreatedSidecarState::StoppedExactCleanupOnly) ||
                            (!no_object && !stopped &&
                             evidence.state != RecreatedSidecarState::Validated)) {
                            error =
                                "fresh-sidecar callback received incomplete or composed evidence";
                            return false;
                        }
                        return true;
                    },
            ...
            if (sidecar_recreated.prerequisite_failure || !sidecar_recreated.success ||
                ...
                sidecar_recreated.semantic_receipt != receipt ||
                sidecar_recreated.error != receipt) {
    ...
    {
        PhaseScope phase_complete_generation_receipt("complete_generation_receipt");
    ...
    {
        PhaseScope phase_callback_failure_cleanup("callback_failure_cleanup");
        bool failing_callback_ran = false;
        const RunResult callback_failure =
            rut::test::ipv4_topology::run_with_held_topology_and_sidecar(
        ...
        if (callback_failure.prerequisite_failure || callback_failure.success ||
            ...
            callback_failure.error != "injected sidecar callback failure" ||
    ...
    {
        PhaseScope phase_sidecar_revalidation_matrix("sidecar_revalidation_matrix");
        for (const auto& fault :
             {std::pair{HeldNamespaceSidecarRevalidationFault::Token, "token"},
              ...
              std::pair{HeldNamespaceSidecarRevalidationFault::NetworkNamespace,
                        "network-namespace"},
              ...
              std::pair{HeldNamespaceSidecarRevalidationFault::NoNewPrivileges,
                        "no-new-privileges"},
              std::pair{HeldNamespaceSidecarRevalidationFault::PublishedPorts,
                        "published-ports"}}) {
