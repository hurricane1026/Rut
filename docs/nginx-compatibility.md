...
| nginx default proxied response buffering/commit | implicit | no; #271 blocks faithful lowering | partial: ordinary RUT carries `complete_content_length` through compiler/JIT/config and executes a bounded cleartext io_uring bodyless known-non-HEAD exact/ANY, strict-200-positive-CL commit barrier; exact GET/POST/PUT/DELETE/PATCH/OPTIONS and ANY are admitted, TRACE only through ANY, while HEAD/CONNECT/Other/OPTIONS-star reject. Full completion, inactivity, and clean-EOF dispositions are implemented, and the profile admits omission or the fixed HTTP/1.1 strip request policy, but it still excludes request bodies and broader response framing/status/size/transports | metadata/forgery/admission plus internal policy-0/policy-1 method-matrix/quiet-buffering/completion/timeout/EOF/capacity/ownership/race/retirement/D2 tests; policy-0 and policy-1 ordinary-RUT GET fragmented-completion/reuse, inactivity/header-only, and clean-EOF/prefix wires plus a policy-1 exact-POST fragmented-completion/fresh-origin wire, with every policy-1 wire asserting the exact rewritten request; committed pinned-nginx inactivity, fragmented-completion, and premature-EOF baselines; no public ANY-TRACE buffering wire or converter differential yet | BLOCKED_BY_RUT |
| `proxy_read_timeout` response-read inactivity timing | yes for explicit raw root-location `1s..63s`; all excluded forms/contexts reject explicitly | no; every present or forged model rejects before RUT emission; #270 lowering blocked by #268 broader method/body domain and #271 default-buffering semantics | partial: ordinary RUT carries exact 1..63s metadata and executes the deadline in narrow cleartext io_uring H1.1 domains: bodyless HEAD; known bodyless origin-form non-HEAD GET/POST/PUT/DELETE/PATCH/OPTIONS/TRACE with exact CL0 success; bounded positive unique-CL POST/PUT/PATCH uploads through the fixed request policy; and bodyless exact/ANY GET with exact 200/positive CL when the whole origin response fits the 16 KiB slice. Post-commit failures close/truncate rather than emit 502/504. This immediate-streaming path matches an explicit `proxy_buffering off` control, not nginx's default buffered small-body stall. Other methods, larger/chunked/close-delimited bodies, broader nginx body semantics, and transports remain unavailable; #268 and #271 stay open | parser/model/rejection coverage; compiler/config and focused method/profile/upload/positive-CL ownership/copy-witness/send-cancel ledger/arm/refresh/expiry/race/capacity/continuation/retirement regressions; production ordinary-source HEAD/GET timing matrix, positive-CL GET fragmented-progress success, exact bodyless POST timeout/fresh-origin-CL0, ANY-route TRACE fragmented-success, fixed-upload POST upload-boundary/fragmented-success/progress-stall evidence, and manual pinned-nginx buffering on/off baselines; no default-buffering RUT equivalence, converter golden, or nginx differential | NOT_IMPLEMENTED |
