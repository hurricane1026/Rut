...
| `proxy_read_timeout` response-read inactivity timing | no | no | partial: ordinary RUT carries exact 1..63s metadata and executes only the cleartext io_uring H1.1 bodyless-HEAD, one-fresh-IPv4, complete-first-header-batch deadline; no positive-progress rearm/refresh or broader transport/request domain; #268 | compiler/config plus focused internal ownership, arm, expiry, race, and fail-closed regressions; no public source timing wire or nginx differential | BLOCKED_BY_RUT |
