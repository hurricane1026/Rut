...
| one exact literal `location = /static` returning status 200 with a 1..64-byte token-safe quoted ASCII body beside the accepted root proxy; bodyless origin-form GET behavior in bounded cleartext H1.1 | yes for this exact two-location model; #286 closed | yes: emits a method-omitted ordinary RUT exact strict-local route beside the unchanged root fallback | yes for the declared literal exact strict-local GET domain; #287 and #288 closed | golden plus AST/HIR/MIR/RIR/RouteConfig ownership; converter-generated explicit-close GET `/static` pinned-nginx differential with zero upstream effects; causal one-send root GET → exact `/static` GET differential proves Date-only-equal 502 keep-alive → 200 close/EOF, zero tail, and exactly one upstream attempt per side | SUPPORTED |
