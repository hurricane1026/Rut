...
| nginx default proxied response buffering/commit | implicit | no; #271 blocks faithful lowering; generic lexer-capacity prerequisite #274 is closed | partial: ordinary RUT carries `complete_content_length` through compiler/JIT/config and executes a bounded cleartext io_uring strict-200-positive-CL commit barrier for the bodyless known-non-HEAD exact/ANY domain and, under exactly the fixed HTTP/1.1 strip policy, bounded positive fixed-CL POST/PUT/PATCH uploads on matching exact/ANY routes. Fragmented completion, inactivity, later clean EOF, and initial coalesced complete-response plus same-batch clean-EOF arbitration are implemented. Validated SocketCreate, ConnectSubmit, and asynchronous ConnectCompletion failures select pinned default 502; canonical per-request response neutrality and a sequential ordinary-RUT unavailable-upstream wire close the discovered #267/#275 sub-gaps. #276 deterministically and on production io_uring preserves a late-arriving pipelined successor into ordinary local dispatch. #277 narrow phase 1 admits a bodyless exact strict GET already coalesced with one ordinary-static successor, with deterministic and ordinary-source production proof. #278 now deterministically activates the same strict policy for one already-dispatched depth-one exact-GET successor, with either default keep-alive or one parser-normalized explicit close, across all ownership boundaries; its ordinary-source production wire and converter differential remain absent, while broader #277/#278 domains remain blocked. HEAD/CONNECT/Other/OPTIONS-star, other request-body methods/framing/capacity, and broader response framing/status/size/transports remain excluded | metadata/forgery/admission plus internal policy-0/policy-1 method/upload/body-wait/quiet-buffering/completion/timeout/EOF/capacity/ownership/race/retirement/D2 tests; the initial coalesced complete+EOF deterministic matrix covers all timeout/CQE orders and persistence modes, and an ordinary policy-1 production wire proves keep-alive then explicit-close over fresh immediately-closing origin episodes; pre-CQE and async failure injection prove pinned 502 across both downstream Recv shapes, including stale/duplicate fencing and health exactly once. Deterministic and production same-forward 502→502 prove neutral sequential reuse and two async health episodes; #276 adds a raw-SQ/CQ production barrier proving a distinct late request reaches ordinary JIT 200 close/EOF after configured 502 keep-alive. #277 phase 1 adds a pre-dispatch `R1||R2` witness followed by exact configured 502 keep-alive, ordinary-static 200 close, EOF, and settled accounting. #278 adds generation-bound deterministic configured-policy execution, exact-close persistence, retired-postcommit, timeout/D2, copied-prebuilt, ordinary-token neutrality, and forgery matrices, but no same-forward production wire or nginx-vs-generated-RUT claim yet. No broader #277/#278 same-forward pipelined-successor claim, enabled broad default-buffering converter golden, or broad nginx-vs-generated-RUT differential is committed | BLOCKED_BY_RUT |
| nginx default buffering for bounded root bodyless origin-form GET, cleartext H1.1, sequential downstream requests only, no CL/TE/Expect/Upgrade, strict 200 with one positive bounded CL | implicit | yes; exact GET lowering restored in `caa9fce3`, with HEAD/ANY and `/api/` isolated | yes for this exact sequential RUT domain: initial same-batch complete+EOF settlement, validated SocketCreate/ConnectSubmit/async ConnectCompletion 502, canonical sequential response neutrality, and the ordinary unavailable-upstream wire are implemented. #276 closes one late-arrival 502-to-ordinary-local pipeline slice, but this row remains explicitly sequential; #277/#278 and broader pipelining are excluded | golden plus AST/HIR/MIR/RIR/RouteConfig ownership passes. Converter-generated pinned-nginx differentials prove complete fixed-CL response plus immediate origin EOF, exact downstream response/EOF and live no-retry settle, and strictly sequential same-downstream unavailable-upstream 502 keep-alive then 502 close/EOF. Deterministic and ordinary-RUT production evidence proves the corresponding settlement and two failure/health episodes | SUPPORTED |
...
- Pipelined validated-failure support has three distinct tracks. Closed #276
  ...
  ordinary JIT dispatch. #277 narrow phase 1 now admits a bodyless exact strict
  GET already coalesced with one ordinary-static successor and proves it through
  deterministic plus ordinary-source production evidence; broader coalesced
...
- The post-#273 audit identified an exact-HEAD streaming, exact-bodyless-GET
  ...
  arrival 502-to-ordinary-local slice, #277 proves only its narrow pre-admission
  exact-GET-to-ordinary-static phase, and broader #277/#278 remain open. `/api/`
  remains separately blocked because complete buffering rejects target
  transforms.
