...
| nginx pre-location rejection for one fresh depth-zero cleartext H1.1 origin-form explicit-close header-absent `TRACE /static` with the accepted root proxy plus exact local-return configuration | implicit in the accepted exact model | no; #302. A TRACE-specific route, unmatched fallback, or converter workaround would not preserve nginx precedence | partial; #303. Generic concrete-only `pre_route METHOD { return local_response(...) }` now survives parser/AST/HIR/MIR/RIR/verifier/printer and transactionally populates the reviewed internal RouteConfig/H1/H2 activation. Ordinary-source production wire evidence is still absent | pinned nginx 1.29.7 baseline under both declaration orders proves the fixed ordered 405/CL157/full-nginx-body/close/EOF wire, one scoped access-log rejection, and zero TRACE upstream attempts; no ordinary-source production or converter equivalence exists | BLOCKED_BY_RUT |
