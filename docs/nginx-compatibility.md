...
| complete existing 2..62-byte clean non-root exact-path class excluding reserved exact `/old`, with literal bodyless `return 204;`, optional meaningful trailing slash, accepted root proxy and the same fresh bodyless explicit-close H1.1 GET domain; #330 | yes structurally: one generic bounded grammar with complete borrowed provenance, `/x`, mixed-alphabet multi-segment and trailing-slash examples, 62/63-byte boundary, empty/dot-segment/dirty/root/variable/control/non-ASCII exclusions and action isolation | partial: provenance-first validation and byte copying are generic, capacities/forgeries/ownership pass, but current view selection emits Raw for every non-trailing key and SlashNormalized only for trailing keys. For a non-trailing multi-segment exact key, nginx default URI normalization makes an embedded doubled-slash request a local hit while generated Raw RUT falls through; #330 converter fix required | yes: generic checked SlashNormalized exact selection from #319 and strict NoContent204 from #323 can express the required behavior; no new RUT capability issue | independently approved pinned-nginx max-boundary oracle `b3095fe6` now proves both 62-byte seams and the blocking embedded-`//` local hit under both declaration orders, with exact wire/EOF, upstream, history, access/log/resource and mutation evidence; Sol focused gates and complete nginx 45/45 pass. A generic lowering fix and separate four-side max-boundary differential are still required before the complete class can be equivalent | PARTIAL |
