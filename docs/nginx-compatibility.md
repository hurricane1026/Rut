...
| one bounded clean exact local-return path `/healthz` beside the accepted root proxy, fresh bodyless explicit-close H1.1 GET domain | yes: non-root absolute clean ASCII-unreserved segments through the 62-byte exact-binding limit with optional single trailing slash, exact borrowed path/body/location/response spans and redirect-action isolation; `af7ff351` | no: unchanged converter explicitly rejects `/healthz` at its genuine path span | yes: generic exact strict-local binding supports clean paths up to 62 bytes | independently approved pinned-nginx-only oracle `59870bb5` proves both declaration orders and five vectors: exact local `/healthz` plus query with live/settled zero upstream, exact root forwarding for `/healthz/`, `/health`, `/`, complete Date-normalized wires/EOF, three ordered backend histories, and five scoped access records per order; focused 2/2 and full pinned-nginx 29/29 pass | PARTIAL |
