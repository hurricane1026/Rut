...
| complete existing 2..62-byte clean non-root exact-path class excluding reserved exact `/old`, with literal bodyful `return 200 "ok";`, optional meaningful trailing slash, accepted root proxy, nginx default `merge_slashes on`, and one fresh bodyless explicit-close cleartext H1.1 GET | yes structurally: `P := "/" SEG ("/" SEG)* ["/"]`, each nonempty `SEG` is ASCII alphanumeric/`-._~`, neither `.` nor `..`; complete borrowed path/body/location/response provenance, 2/62 acceptance, 63 rejection, dirty/root/variable/control/non-ASCII exclusions and `/old` action isolation are covered | yes generically in `50aed5d7`: validated path bytes are copied without path-content/length branching into exactly one method-omitted ordinary public `route exact slash_normalized` binding with the fixed Representation200 tuple. Both maximum non-trailing/trailing path plus 64-byte structural body cases are exactly 5681/5937 bytes and compile to owned RouteConfig; this row claims only body `ok` | yes for the declared bounded composition: generic #319 SlashNormalized selection, #332 complete access-target ownership through 128 bytes, #333 positive-CL `{200,201}` complete-buffered fallback status and #334 zero/one typed Content-Type. Percent/dot normalization, leading slash runs, `merge_slashes off`, broader methods/bodies/framing/protocols and response semantics remain excluded | class-level consolidation of independently reviewed evidence: #318 proves bodyful non-trailing literal/query/miss behavior, #320 bodyful configured trailing slash-run behavior, #331 bodyful multi-segment normalized selection plus exact 201/Content-Type fallback in both declaration orders, and #330/#332 directly prove both 62-byte path shapes, the 66-byte `<key>?x=1` access target and normalized-buffer boundary on the same path/matcher/access pipeline. Source audit finds no path-literal or length/action coupling, so no duplicate max-boundary differential is required | SUPPORTED |
