...
| one exact `location = /old { return 301 http://redirect.example/new; }` beside the accepted root proxy, for one bounded fresh explicit-close GET | no; #307 | no; current exact action only accepts bounded `/static` return-200 body | no; #308 tracks the required generic fixed-authority/omit-port/discard-query/typed-order redirect policy | manual pinned nginx 1.29.7 evidence under both declaration orders fixes the 366-byte Date-normalized wire, fixed Location, query discard, close/EOF, access log and zero upstream; committed oracle, ordinary-source production, golden and converter differential remain absent | BLOCKED_BY_RUT |
