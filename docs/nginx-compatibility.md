...
| one exact literal `location = /static` returning exact 200 beside the accepted root proxy | no; #286 | no; #286 | partial: strict nginx-shaped status-200 response is proven and #287 closed; #288 exact-terminator compiler/runtime activation and depth-zero ordinary-source production selection are implemented, but request-boundary production and nginx-vs-hand-authored-RUT evidence D remain | pinned nginx exact-location baseline, ordinary-RUT top-level response differential, and ordinary exact-route production wire; no exact-route cross-runtime differential or generated-converter routing evidence | BLOCKED_BY_RUT |
