...
| exact bodyless `location = /static { return 204; }` beside the accepted root proxy, fresh bodyless explicit-close H1.1 GET only | no: current local-return parser rejects status 204 at its status span | no: no distinct no-content semantic action or faithful lowering | no: #323 tracks a generic derived strict `NoContent204` profile. Current strict local response rejects 204; ordinary `return 204` emits CL0 and omits Server/Date | independently reviewed investigation proves both pinned-nginx declaration orders emit the exact 105-byte status/Server/Date/Connection-only wire, EOF and zero upstream; no committed oracle or converter differential | BLOCKED_BY_RUT |
