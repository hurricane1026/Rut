...
| one exact literal `location = /static` returning exact 200 beside the accepted root proxy | no; #286 | no; #286 | partial: strict nginx-shaped status-200 response is proven and #287 closed; #288 exact-terminator compiler/runtime activation is implemented, but ordinary production and nginx-vs-hand-authored-RUT evidence D is not complete | pinned nginx exact-location baseline plus ordinary-RUT top-level response differential; no exact-route production/differential or generated-converter routing evidence | BLOCKED_BY_RUT |
