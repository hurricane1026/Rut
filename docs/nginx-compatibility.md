...
| the same bounded exact/root-proxy shape with `return 302 http://redirect.example/new;`, for one fresh explicit-close GET | yes: strict literal `{301,302}` bounded model with genuine borrowed status lexeme/span/provenance, both declaration orders and forged/unsupported-shape rejection; `86144bfa` | no: the unchanged converter explicitly rejects parsed 302 with no RUT output; #313 lowering is CURRENT | yes for the generic bounded fixed-target policy: closed `{301,302}` validator, frontend/JIT/loader/wire/real-io_uring proof and full acceptance are complete in `427ba484`; legacy request-host and all wider status/tuple shapes are unchanged; #312 closed | committed pinned nginx 1.29.7 oracle `e9d67fff` proves both declaration orders and five vectors: exact 342-byte 302 response/EOF, Host independence/query discard, live/settled zero upstream, exact forward histories and five scoped access records per order; no converter differential | PARTIAL |
