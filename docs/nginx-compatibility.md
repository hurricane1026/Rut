...
| `proxy_read_timeout` response-read inactivity timing | no; must reject explicitly | no; #270 | partial: ordinary RUT carries exact 1..63s metadata and, in the cleartext io_uring H1.1 bodyless-HEAD, one-fresh-IPv4 domain, executes the first-response deadline plus exact positive-header-progress refresh/rearm with deterministic batch arbitration; broader transport/request domains remain unavailable and #268 blocks #270 | compiler/config and focused ownership/copy-witness/ledger/arm/refresh/expiry/race regressions, plus a production ordinary-source 1s zero-byte-origin wire with downstream reuse; fragmented production-wire and nginx differential evidence remain absent | BLOCKED_BY_RUT |
