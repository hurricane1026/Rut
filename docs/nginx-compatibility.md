...
| exact bodyless `location = /static { return 204; }` beside the accepted root proxy, fresh bodyless explicit-close H1.1 GET only | no: current local-return parser rejects status 204 at its status span | no: no distinct no-content semantic action or faithful lowering | partial: #323 Stage 1 `935255f6` defines the complete-tuple internal `NoContent204` contract, Stage 2 `920a4747` propagates it only through explicitly named synthetic compiler/config paths with atomic owned publication, Stage 3a `c858ee44` safely settles synchronous strict-local send failures, and Stage 3b1 `4b6a03a7` proves the exact internal header-only close/keep-alive serializer and capacity boundaries without a production caller. Public source/loader still reject and the runtime zero-send fence remains closed; activation plus production evidence are pending, and ordinary `return 204` remains non-equivalent CL0 | independently reviewed investigation proves both pinned-nginx declaration orders emit the exact 105-byte status/Server/Date/Connection-only wire, EOF and zero upstream; Stage 1–3b1 focused normal/sanitizer gates and the post-3b1 full Network 1041/1041 pass, but no committed nginx oracle or converter differential exists | BLOCKED_BY_RUT |
