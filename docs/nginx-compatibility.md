...
| nginx pre-location rejection for one fresh depth-zero cleartext H1.1 origin-form explicit-close header-absent `TRACE /` with the accepted root-only proxy configuration | pending pinned oracle; #305 | no; root-only model intentionally does not activate the closed implicit TRACE profile | yes: generic bounded `pre_route TRACE` capability #303 is closed, subject to oracle match | no; exact-root evidence does not prove root-only converter behavior | NOT_IMPLEMENTED |
