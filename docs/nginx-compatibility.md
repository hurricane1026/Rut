...
| exact bodyless `location = /static { return 204; }` beside the accepted root proxy, fresh bodyless explicit-close H1.1 literal `GET /static` only; #324 closed | yes for the literal bounded action: distinct borrowed `ExactNoContentReturnLocation`, exact `/static`, literal status 204 with complete spans/provenance and explicit exclusions; `a0427b82` | yes: provenance-first complete lexical-shell validation emits one canonical ordinary public `route exact GET "/static"` with the complete strict no-content tuple; dual-order/format full goldens, exact 5539/5547 capacity, forgeries and source-destruction owned RouteConfig pass; `184dbb25` | yes for the bounded target: closed #323 stages through public activation and real CLI/io_uring proof (`935255f6`, `920a4747`, `c858ee44`, `4b6a03a7`, `d3abaa6b`, `fad6be75`) preserve the exact 105-byte no-content profile, ordinary-204 CL0 distinction, zero upstream and selector/fallback isolation | approved oracle `97ed1750` and converter-generated four-side differential `62dcbea4` prove both declaration orders over eight unique ports: genuine parse/lower, public CLI/io_uring source destruction, pairwise exact Date-normalized 105-byte local wires/EOF, live/settled zero upstream, exact 118-byte fallback wires and one byte-exact episode/no retry, exactly two ordered response-size access records and clean lifecycle logs per side. Sol focused/repeated closure gates and complete nginx 38/38 pass | SUPPORTED |
| the same exact bodyless `return 204;` config for one fresh explicit-close H1.1 literal `GET /static?x=1`; #325 closed | implicit in the accepted distinct action; request query does not alter nginx config parsing | yes: existing canonical exact GET route is unchanged; generated source is byte-identical across declaration orders after verified port-only normalization and explicitly rejects query/Host-specific source markers; `f2c7c504` | yes: generic exact selection treats `?` as the path boundary and ordinary public NoContent204 runs through real CLI/io_uring; no new RUT primitive or issue is needed | approved pinned oracle `bddf01c3` and final converter-generated four-side differential `f2c7c504` prove both declaration orders over eight unique ports: genuine parse/lower, source destruction, pairwise exact Date-normalized 105-byte wire/EOF, live/settled zero upstream, exact 118-byte fallback/one episode/no retry, two ordered overflow-safe access records and clean five-line RUT lifecycles. Sol repeated/focused gates and complete nginx 40/40 pass | SUPPORTED |
| bounded clean exact bodyless `location = /healthz { return 204; }` beside the accepted root proxy; fresh bodyless explicit-close H1.1 GET domain; #328 closed | yes: literal bodyless 204 reuses the existing bounded clean 62-byte exact-path grammar with complete borrowed path/location/directive/status spans and provenance in either order; clean multi-segment/trailing-slash, 62/63 boundaries, dirty/root/variable/non-ASCII exclusions, `/old` action isolation and `/static` regression pass; `9c3c67f6` | yes: provenance-first validator admits only the modeled 2..62 clean path and emits ordinary public Raw/SlashNormalized exact GET with the unchanged strict NoContent204 tuple; `/static` full golden/5539 and max-port 5547 remain exact, `/x` lower bound, `/healthz` dual-order full golden, 62-byte 5602/5619 capacity, forgeries and source-destruction AST→HIR→MIR→RIR→owned RouteConfig pass; `bc846bee` | yes in generic production: ordinary public exact GET plus strict NoContent204 and root forwarding compile, own and execute through real CLI/io_uring; no new RUT capability issue | approved pinned-nginx oracle `85749c79` plus mutation hardening `7cb099d0` and final converter-generated four-side differential `8d073885` prove both declaration orders across eight unique ports/four isolated nginx/RUT sides. Genuine parse/lower and public CLI/io_uring execution match five exact Date-normalized wires/EOF: 105-byte local `/healthz` and query bodyless 204 with live/settled zero upstream, plus 118-byte slash/sibling/root fallbacks and three ordered byte-exact upstream histories/no fourth. Five ordered response-size access records, exact lifecycle logs, source destruction/ownership and fail-closed source/access/resource/port mutations pass. A full-suite readiness TOCTOU was repaired with reviewed `SOCK_CLOEXEC` bind reservations without retry or semantic relaxation; post-fix stress 10/10 and Sol complete nginx 42/42 in 177.50s pass | SUPPORTED |
...
| complete existing 2..62-byte clean non-root exact-path class excluding reserved exact `/old`, with literal bodyless `return 204;`, optional meaningful trailing slash, accepted root proxy and the same fresh bodyless explicit-close H1.1 GET domain; #330 | yes structurally: one generic bounded grammar with complete borrowed provenance, `/x`, mixed-alphabet multi-segment and trailing-slash examples, 62/63-byte boundary, empty/dot-segment/dirty/root/variable/control/non-ASCII exclusions and action isolation | partial: provenance-first validation and byte copying are generic, capacities/forgeries/ownership pass, but the current released view selection emits Raw for every non-trailing key and SlashNormalized only for trailing keys. Two independent design audits select a uniform ordinary-RUT SlashNormalized GET binding for every validated NoContent204 exact key; implementation/review remain CURRENT. For a non-trailing multi-segment key, this is required because nginx default URI normalization makes an embedded doubled-slash request a local hit while current Raw RUT falls through | yes: generic checked SlashNormalized exact selection from #319 and strict NoContent204 from #323 can express the required behavior; no new RUT capability issue | independently approved pinned-nginx max-boundary oracle `b3095fe6` proves both 62-byte seams and the blocking embedded-`//` local hit under both declaration orders, with exact wire/EOF, upstream, history, access/log/resource and mutation evidence; Sol focused gates and complete nginx 45/45 pass. The reviewed generic lowering fix and separate four-side max-boundary differential are still required before the complete class can be equivalent | PARTIAL |
