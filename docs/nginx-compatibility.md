...
| nginx pre-location rejection for one fresh depth-zero cleartext H1.1 origin-form explicit-close header-absent `TRACE /static` with the accepted root proxy plus exact local-return configuration | implicit in the accepted exact model | no; #302. A TRACE-specific route, unmatched fallback, or converter workaround would not preserve nginx precedence | partial internal activation; #303. Hand-built RouteConfig tests prove a generic concrete-method pre-route strict response before exact/prefix/unmatched, bounded H1 fail-close, transactional ownership, and H2 match fencing. Ordinary `.rut` syntax/IR/loader and production evidence are still absent | pinned nginx 1.29.7 baseline under both declaration orders proves the fixed ordered 405/CL157/full-nginx-body/close/EOF wire, one scoped access-log rejection, and zero TRACE upstream attempts; no ordinary-source RUT or converter equivalence exists | BLOCKED_BY_RUT |
