...
| the same exact local return for one fresh depth-zero cleartext H1.1 origin-form explicit-close header-absent `PATCH /static` | implicit in the accepted exact model | yes: existing method-omitted exact lowering is sufficient; converter differential pending in #300 | yes for this exact fresh generic request slice: the closed DELETE/PUT/PATCH proof, deterministic lifecycle/exclusion evidence, and ordinary-source frontend/JIT/production-io_uring wire are complete; #301 closed. Query, framing including CL0, body/tail, reuse/pipeline, TLS/H2, and proxy PATCH remain excluded | pinned nginx 1.29.7 baseline under both declaration orders and ordinary-RUT production independently prove exact 200/CL16/full-body/close/EOF and zero upstream; converter-generated nginx-vs-RUT differential pending | PARTIAL |
