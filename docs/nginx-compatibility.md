...
| bounded clean exact bodyless `location = /healthz { return 204; }` beside the accepted root proxy; fresh explicit-close H1.1 GET domain; #328 | yes: literal bodyless 204 reuses the existing bounded clean 62-byte exact-path grammar with complete borrowed path/location/directive/status spans and provenance in either order; clean multi-segment/trailing-slash, 62/63 boundaries, dirty/root/variable/non-ASCII exclusions, `/old` action isolation and `/static` regression pass; `9c3c67f6` | yes: provenance-first validator admits only the modeled 2..62 clean path and emits ordinary public Raw/SlashNormalized exact GET with the unchanged strict NoContent204 tuple; `/static` full golden/5539 and max-port 5547 remain exact, `/x` lower bound, `/healthz` dual-order full golden, 62-byte 5602/5619 capacity, forgeries and source-destruction AST→HIR→MIR→RIR→owned RouteConfig pass; `bc846bee` | yes in generic production: ordinary public exact GET plus strict NoContent204 and root forwarding compile, own and execute through real CLI/io_uring; no new RUT capability issue | approved pinned-nginx oracle `85749c79` plus mutation hardening `7cb099d0` prove both orders and five vectors: exact Date-normalized 105-byte local `/healthz` and query wires/EOF with live/settled zero upstream; exact 118-byte slash/sibling/root fallback wires and three ordered byte-exact upstream histories/no fourth; five bounded access records and nine fail-closed evidence mutations. Sol repeated/focused gates and complete nginx CTest 35/35 pass. Converter-generated four-side differential remains absent | PARTIAL |
