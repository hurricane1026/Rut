...
| nginx pre-location rejection for one fresh depth-zero cleartext H1.1 origin-form explicit-close header-absent `TRACE /api/x` with the accepted `/api/` proxy-URI replacement configuration | nginx 1.29.7 oracle pinned; semantic profile activation remains absent; #306 | no; `/api/` model intentionally does not yet activate the closed implicit TRACE profile | yes: the oracle matches closed generic bounded `pre_route TRACE` capability #303 | pinned nginx-only required-mode test proves the fixed ordered 405/CL157/full-nginx-body/close/EOF wire, one scoped access record, and live/settled zero upstream; no converter-generated RUT evidence exists | NOT_IMPLEMENTED |
