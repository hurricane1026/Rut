...
| nginx default proxied response buffering/commit | implicit | no; #271 blocks faithful lowering; generic lexer-capacity prerequisite #274 is closed | partial: ordinary RUT carries `complete_content_length` through compiler/JIT/config and executes a bounded cleartext io_uring strict-200-positive-CL commit barrier for the bodyless known-non-HEAD exact/ANY domain and, under exactly the fixed HTTP/1.1 strip policy, bounded positive fixed-CL POST/PUT/PATCH uploads on matching exact/ANY routes. Fragmented completion, inactivity, later clean EOF, and initial coalesced complete-response plus same-batch clean-EOF arbitration are implemented. Validated SocketCreate, ConnectSubmit, and asynchronous ConnectCompletion failures select pinned default 502; canonical per-request response neutrality and a sequential ordinary-RUT unavailable-upstream wire close the discovered #267/#275 sub-gaps. Pipelined validated-failure successors separately lose already-received bytes (#276). HEAD/CONNECT/Other/OPTIONS-star, other request-body methods/framing/capacity, and broader response framing/status/size/transports remain excluded | metadata/forgery/admission plus internal policy-0/policy-1 method/upload/body-wait/quiet-buffering/completion/timeout/EOF/capacity/ownership/race/retirement/D2 tests; the initial coalesced complete+EOF deterministic matrix covers all timeout/CQE orders and persistence modes, and an ordinary policy-1 production wire proves keep-alive then explicit-close over fresh immediately-closing origin episodes; pre-CQE and async failure injection prove pinned 502 across both downstream Recv shapes, including stale/duplicate fencing and health exactly once. Deterministic and production same-forward 502→502 prove neutral sequential reuse and two async health episodes; no pipelined-successor claim, enabled default-buffering converter golden, or nginx-vs-generated-RUT differential is committed | BLOCKED_BY_RUT |
| nginx default buffering for bounded root bodyless origin-form GET, cleartext H1.1, sequential downstream requests only, no CL/TE/Expect/Upgrade, strict 200 with one positive bounded CL | implicit | no; the attempted exact-GET lowering in `d5fc3d51` was disabled by `efefc901` after it regressed existing success/gateway behavior | yes for this exact RUT domain: initial same-batch complete+EOF settlement, validated SocketCreate/ConnectSubmit/async ConnectCompletion 502, canonical sequential response neutrality, and the ordinary unavailable-upstream wire are implemented; pipelined validated-failure successors remain #276 | rejected draft converter differential exposed both original zero-byte paths; rollback preserves prior converter behavior. Both gaps now have deterministic and ordinary-RUT production evidence, including sequential 502→502; restored converter golden and nginx-vs-generated-RUT completion/gateway differentials remain | NOT_IMPLEMENTED |
