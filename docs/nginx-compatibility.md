...
| the same exact local return for one fresh depth-zero cleartext H1.1 origin-form explicit-close header-absent `PATCH /static` | implicit in the accepted exact model | yes: existing method-omitted exact lowering is sufficient; #300 | no: parser/method key/exact ANY selector and serializer represent PATCH, but the closed strict fresh-method proof currently admits only DELETE and PUT; #301 | pinned nginx 1.29.7 baseline under both declaration orders proves exact 200/CL16/full-body/close/EOF and zero upstream; RUT production and converter differential pending | BLOCKED_BY_RUT |
