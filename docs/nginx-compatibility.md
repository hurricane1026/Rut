...
| nginx default proxied response buffering/commit | implicit | no; #271 blocks faithful lowering | no: current bounded positive-CL path streams immediately and matches only explicit `proxy_buffering off`; it cannot reproduce default buffering on timeout | manual pinned-nginx 1.29.7 on/off baselines; no committed RUT equivalence test | BLOCKED_BY_RUT |
