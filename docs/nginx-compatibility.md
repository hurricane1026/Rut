...
| nginx default buffering for bounded root bodyless origin-form GET, cleartext H1.1, sequential downstream requests only, no CL/TE/Expect/Upgrade, strict 200 with one positive bounded CL | implicit | yes; exact GET lowering restored in `caa9fce3`, with HEAD/ANY and `/api/` isolated | yes for this exact RUT domain: initial same-batch complete+EOF settlement, validated SocketCreate/ConnectSubmit/async ConnectCompletion 502, canonical sequential response neutrality, and the ordinary unavailable-upstream wire are implemented; pipelined validated-failure successors remain #276 | golden plus AST/HIR/MIR/RIR/RouteConfig ownership passes. The rejected draft converter differential exposed both original zero-byte paths; both gaps now have deterministic and ordinary-RUT production evidence, including sequential 502→502. Converter-generated nginx-vs-RUT completion and gateway differentials remain | NOT_IMPLEMENTED |
