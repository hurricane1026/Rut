...
| one exact `location = /old { return 301 http://redirect.example/new; }` beside the accepted root proxy, for one bounded fresh explicit-close GET | no; #307 | no; current exact action only accepts bounded `/static` return-200 body | under investigation: generic redirect and exact selection exist separately, but exact nginx wire/Location/body production composition is unproven | no pinned oracle, ordinary-source production, golden, or converter differential | NOT_IMPLEMENTED |
