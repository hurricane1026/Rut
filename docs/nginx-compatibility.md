...
| the same exact local return for one fresh depth-zero cleartext H1.1 origin-form explicit-close header-absent `PUT /static` | implicit in the accepted exact model | yes: existing method-omitted exact lowering is sufficient; #298 | no: parser/method key/exact ANY selector and serializer represent PUT, but strict exact-local admission rejects it after selection; generic capability #299 | pinned nginx semantics and all RUT/converter behavior evidence pending | BLOCKED_BY_RUT |
