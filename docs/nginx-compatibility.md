...
| nginx default proxied response buffering/commit | implicit | no; #271 blocks faithful lowering | partial metadata only: ordinary RUT carries `complete_content_length` through compiler/JIT/config, but runtime deliberately rejects before effects; current executable positive-CL path still matches only explicit `proxy_buffering off` | metadata/forgery/pre-effect-rejection tests and manual pinned-nginx 1.29.7 on/off baselines; no runtime or equivalence wire | BLOCKED_BY_RUT |
