...
| exact bodyless `location = /static { return 204; }` beside the accepted root proxy, fresh bodyless explicit-close H1.1 GET only | no: current local-return parser rejects status 204 at its status span | no: no distinct no-content semantic action or faithful lowering | partial: #323 Stage 1 `935255f6` defines the complete-tuple internal `NoContent204` contract and Stage 2 `920a4747` propagates it only through explicitly named synthetic compiler/config paths with atomic owned publication. Public source/loader still reject, the runtime zero-send fence remains closed, and serializer plus production evidence are pending; ordinary `return 204` remains non-equivalent CL0 | independently reviewed investigation proves both pinned-nginx declaration orders emit the exact 105-byte status/Server/Date/Connection-only wire, EOF and zero upstream; Stage 1/2 focused normal and sanitizer gates pass, but no committed nginx oracle or converter differential exists | BLOCKED_BY_RUT |
