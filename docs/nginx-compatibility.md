...
| exact trailing-slash bodyless `location = /healthz/ { return 204; }` beside the accepted root proxy; proposed fresh bodyless explicit-close H1.1 GET slice; #329 | yes in the existing bounded model: trailing slash is preserved with complete borrowed path/location/directive/status spans and provenance; no broader normalization syntax is claimed | yes in the existing lowering: the same provenance-first 2..62-byte clean-path validator emits ordinary public `route exact slash_normalized GET "/healthz/"` with the unchanged strict NoContent204 tuple; no nginx-specific runtime mode | yes: generic checked `slash_normalized` exact selection from #319/#320 composes with generic strict NoContent204 from #323; query exclusion and slash-run collapse are ordinary RUT primitives | independently approved required pinned-nginx oracle `3975fff1` proves both declaration orders and five raw targets across four bind-reserved ports: `/healthz/`, its query and `/healthz//` emit exact Date-normalized 105-byte bodyless 204 wires/EOF with live/settled zero upstream; `/healthz` has no redirect and it plus `/` emit exact fallback wires and two ordered byte-exact histories/no third. Five bounded access records, clean logs and fail-closed evidence/resource mutations pass; Sol focused gates and complete nginx 43/43 pass. Converter-generated four-side differential remains absent | PARTIAL |
