...
| the same bounded exact local return with body `"hello world"`, containing exactly one non-edge ASCII space, fresh bodyless explicit-close H1.1 GET only | no: current lexer splits the quoted body at the internal space; #321 | no; contextual raw quote scanning, borrowed span/provenance and strict rejection boundaries are planned in #321 | yes: ordinary RUT `b"hello world"` compiles, owns and serves the exact wire through the public CLI/io_uring; no RUT capability issue is needed | independently approved pinned nginx 1.29.7 oracle `c70c8c32` proves both isolated declaration orders return the exact Date-normalized 154-byte 200/CL11/`hello world`/close/EOF wire with no newline/tail/Location, live and settled zero upstream, and one raw access record per order. No converter golden or converter-generated differential exists yet | NOT_IMPLEMENTED |
