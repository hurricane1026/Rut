...
| exact bodyless `location = /static { return 204; }` beside the accepted root proxy, fresh bodyless explicit-close H1.1 GET only | no: current nginx local-return parser rejects status 204 at its status span | no: no distinct nginx no-content semantic action or lowering yet | partial: #323 Stages 1–3b1 (`935255f6`, `920a4747`, `c858ee44`, `4b6a03a7`) define/propagate the exact tuple, settle sends and prove serializer bytes. Stage 3b2 `d3abaa6b` atomically activates only the complete tuple through ordinary public strict-local source/parser/compiler/loader and runtime close/keep lifecycle, with generic whole-config rollback; neighboring statuses/mutations reject and ordinary `return 204` remains distinct CL0. Real public-CLI/io_uring production evidence is still pending in Stage 4 | independently reviewed investigation proves both pinned-nginx declaration orders emit the exact 105-byte status/Server/Date/Connection-only wire, EOF and zero upstream; Stage 3b2 focused normal/sanitizer plus full Frontend 1009/1009, RIR 65/65, RouteConfig 41/41, Loader 37/37 and Network 1044/1044 pass, but no committed ordinary-source io_uring proof, nginx oracle or converter differential exists | BLOCKED_BY_RUT |
