...
| `proxy_read_timeout` response-read inactivity timing | yes for explicit raw root-location `1s..63s`; all excluded forms/contexts reject explicitly | no; every present or forged model rejects before RUT emission; #270 lowering blocked by #268 broader method/body domain | partial: ordinary RUT carries exact 1..63s metadata and executes the deadline in narrow cleartext io_uring H1.1 domains: bodyless HEAD; known bodyless origin-form non-HEAD GET/POST/PUT/DELETE/PATCH/OPTIONS/TRACE; and bounded positive unique-CL POST/PUT/PATCH uploads through the fixed request policy. Exact method/upload ownership is preserved and success remains limited to HTTP/1.1 200 with unique `Content-Length: 0`; TRACE requires an ANY route and every other request/response shape fails closed. Successful response bodies and broader nginx body semantics remain unavailable; #268 stays open | parser/model/rejection coverage; compiler/config and focused method/profile/upload/ownership/copy-witness/ledger/arm/refresh/expiry/race/full-timeout-body/retirement regressions; production ordinary-source HEAD/GET timing matrix, exact bodyless POST timeout/fresh-origin-CL0, ANY-route TRACE fragmented-success, and fixed-upload POST no-arm-during-upload/zero-response-timeout plus fragmented-success refresh evidence; no fixed-upload public progress-stall, response-body, converter golden, or nginx differential | NOT_IMPLEMENTED |
