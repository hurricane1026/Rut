...
| one exact `location = /old { return 301 http://redirect.example/new; }` beside the accepted root proxy, for one bounded fresh explicit-close GET | no; #307 | no; current exact action only accepts bounded `/static` return-200 body | yes for the bounded generic fixed redirect profile; #308 closed through metadata/ownership `054d4d23`, activation `2d9ec5b3`, and loader/JIT/io_uring production `bdaecb4d` | pinned nginx oracle `da414a71` and independent ordinary-RUT production proof exist; converter parser/golden and converter-generated differential remain absent | NOT_IMPLEMENTED |
