...
| exact bodyless `location = /static { return 204; }` beside the accepted root proxy, fresh bodyless explicit-close H1.1 GET only | no: current local-return parser rejects status 204 at its status span | no: no distinct no-content semantic action or faithful lowering | partial: #323 Stage 1 `935255f6` defines the complete-tuple internal `NoContent204` contract, explicit reserved-byte ABI and empty-view rules while public/common validation still rejects 204. Compiler/config propagation, serializer and production evidence remain pending; ordinary `return 204` remains non-equivalent CL0 | independently reviewed investigation proves both pinned-nginx declaration orders emit the exact 105-byte status/Server/Date/Connection-only wire, EOF and zero upstream; no committed oracle or converter differential | BLOCKED_BY_RUT |
