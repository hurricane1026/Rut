...
| one exact literal `location = /static` returning exact 200 beside the accepted root proxy | yes for a 1..64-byte token-safe quoted ASCII body; #286 | no; parsed and forged inventory rejects explicitly pending #286 lowering | yes for the bounded hand-authored cleartext H1.1 literal exact strict-local domain; #287 and #288 closed | parser/model and fail-closed converter tests; pinned nginx exact-location baseline, ordinary exact-route depth-zero and bounded depth-one production wires, and pinned-nginx vs hand-authored-RUT D3 differential; no converter-generated exact-location evidence | NOT_IMPLEMENTED |
