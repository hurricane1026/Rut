...
| nginx pre-location rejection for one fresh depth-zero cleartext H1.1 origin-form explicit-close header-absent `TRACE /api/x` with the accepted `/api/` proxy-URI replacement configuration | pending pinned oracle; #306 | no; `/api/` model intentionally does not activate the closed implicit TRACE profile | yes: closed generic bounded `pre_route TRACE` capability #303, subject to oracle match | no; root/exact-root evidence does not prove `/api/` converter behavior | NOT_IMPLEMENTED |
