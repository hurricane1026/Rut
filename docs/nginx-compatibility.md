...
| `OPTIONS *` under the accepted minimal root proxy config | implicit | no; current generated root route cannot match asterisk-form | no faithful target handling: RUT has no canonical path, bypasses forward preflight, and emits fixed unmatched 200 | pinned nginx 1.29.7 exact-close 400/157-byte-body/EOF with zero upstream effects; no generated-RUT differential | BLOCKED_BY_RUT |
...
- Non-origin-form request targets currently receive no RUT canonical path, miss
  ...
  local 400 and zero upstream effects; authority-form CONNECT remains to be
  pinned before choosing a generic RUT routing/unmatched action.
