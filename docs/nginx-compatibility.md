...
- Pinned nginx 1.29.7 also keeps a default-keepalive HEAD reusable after a read
  ...
  Connect, Send, backend dispatch, and TimerWheel state; a public ordinary-source
  real-socket wire and converter-generated differential are still absent. Every
  excluded phase/layout, including epoll, remains zero-byte fail-closed. #268
  separately tracks per-forward read-timeout timing semantics. The broader
