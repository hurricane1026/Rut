...
| one exact literal `/static` local return for a fresh depth-zero cleartext H1.1 origin-form explicit-close POST with exactly one valid `Content-Length: 0`, no other framing/body/tail | implicit in the accepted exact model | yes: the existing method-omitted exact route is sufficient; no CL-specific source or converter workaround; #292 tracks differential closure | yes for this exact generic request slice: field-count identity, single validated numeric-zero POST admission, deterministic lifecycle/forgery coverage, and ordinary-source frontend/JIT/production-io_uring wire are complete; #291 closed. Duplicate/conflicting/positive/malformed CL, CL+TE, other methods, bodies/tails, reuse/pipeline, TLS/H2, and alternate-zero compatibility claims remain excluded | pinned nginx 1.29.7 baseline under both location orders and an independent hand-authored ordinary-RUT production wire each prove exact 200/CL16/body/close/EOF and zero upstream effects; #292 converter-generated nginx-vs-RUT differential still pending | PARTIAL |
...
- Pinned nginx 1.29.7 treats one valid `Content-Length: 0` on the bounded fresh
  ...
  closed. #292 tracks the separate converter-generated differential.
