...
| nginx pre-location rejection for one fresh depth-zero cleartext H1.1 origin-form explicit-close header-absent `TRACE /` with the accepted root-only proxy configuration | yes: closed server-level semantic profile; #305 | yes: emits the same ordinary concrete `pre_route TRACE` before unmatched/routes; `/api/` remains excluded | yes: the pinned oracle matches the closed generic bounded `pre_route TRACE` capability #303 | pinned nginx-only required-mode test proves the fixed ordered 405/CL157/full-nginx-body/close/EOF wire, one scoped access record, and live/settled zero upstream; converter golden and post-lifetime owned RouteConfig evidence pass, but no converter-generated nginx-vs-RUT differential exists | PARTIAL |
