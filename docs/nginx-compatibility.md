...
| nginx default proxied response buffering/commit | implicit | no; #271 blocks faithful lowering; generic lexer-capacity prerequisite #274 is closed | partial: ordinary RUT carries `complete_content_length` through compiler/JIT/config and executes a bounded cleartext io_uring strict-200-positive-CL commit barrier for the bodyless known-non-HEAD exact/ANY domain and, under exactly the fixed HTTP/1.1 strip policy, bounded positive fixed-CL POST/PUT/PATCH uploads on matching exact/ANY routes. Fragmented completion, inactivity, later clean EOF, and initial coalesced complete-response plus same-batch clean-EOF arbitration are implemented. Validated SocketCreate, ConnectSubmit, and asynchronous ConnectCompletion failures select pinned default 502; the async path has deterministic CQE/ownership evidence, while its sequential two-request ordinary-RUT unavailable-upstream wire is blocked by per-request response-state leakage #275. Pipelined validated-failure successors separately lose already-received bytes (#276). HEAD/CONNECT/Other/OPTIONS-star, other request-body methods/framing/capacity, and broader response framing/status/size/transports remain excluded | metadata/forgery/admission plus internal policy-0/policy-1 method/upload/body-wait/quiet-buffering/completion/timeout/EOF/capacity/ownership/race/retirement/D2 tests; the initial coalesced complete+EOF deterministic matrix covers all timeout/CQE orders and persistence modes, and an ordinary policy-1 production wire proves keep-alive then explicit-close over fresh immediately-closing origin episodes; pre-CQE and async failure injection prove pinned 502 across both downstream Recv shapes, including stale/duplicate fencing and health exactly once. The rejected sequential production-wire draft emits request-1 502 but request-2 zero-byte EOF with health one; no false wire, pipelined-successor claim, enabled default-buffering converter golden, or nginx-vs-generated-RUT differential is committed | BLOCKED_BY_RUT |
| nginx default buffering for bounded root bodyless origin-form GET, cleartext H1.1, no CL/TE/Expect/Upgrade, strict 200 with one positive bounded CL | implicit | no; the attempted exact-GET lowering in `d5fc3d51` was disabled by `efefc901` after it regressed existing success/gateway behavior | partial: initial same-batch complete+EOF settlement is fixed in `082f20bd` with deterministic and production-wire evidence; validated SocketCreate, ConnectSubmit, and asynchronous ConnectCompletion use pinned default 502, but reusable async failures leak completed response state into the successor request (#275) | rejected draft completion differential exposed both original zero-byte paths; rollback restores the committed converter-generated success/gateway suite. Focused async evidence passes, while the rejected ordinary-RUT wire proves request 2 still zero-byte closes; #275, the accepted wire, and gateway/converter differentials remain | BLOCKED_BY_RUT |
...
- The post-#273 audit identified an exact-HEAD streaming, exact-bodyless-GET
  ...
  (#267). The first gap is fixed with deterministic and ordinary-RUT public
  evidence; pre-CQE local failures and async completion are fixed, while the
  async ordinary-RUT unavailable-upstream production wire is now blocked by
  reusable successor-state leakage #275. `efefc901` keeps the split disabled
  until #267/#275 evidence and the converter
  differentials also land. `/api/` remains separately blocked because complete
  buffering rejects target transforms.
