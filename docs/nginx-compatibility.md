...
| bounded clean exact bodyless `location = /healthz { return 204; }` beside the accepted root proxy; fresh explicit-close H1.1 GET domain; #328 | no: clean exact-path grammar exists for status 200, but the distinct no-content action currently rejects every path other than literal `/static` | no: the no-content validator independently assumes the `/static` lexical shell; planned composition reuses #318 clean path and #324 action without a special case | yes in generic production: ordinary public exact GET plus strict NoContent204 and root forwarding compile, own and execute through real CLI/io_uring; no new RUT capability issue | approved pinned-nginx oracle `85749c79` plus mutation hardening `7cb099d0` prove both orders and five vectors: exact Date-normalized 105-byte local `/healthz` and query wires/EOF with live/settled zero upstream; exact 118-byte slash/sibling/root fallback wires and three ordered byte-exact upstream histories/no fourth; five bounded access records and nine fail-closed evidence mutations. Sol repeated/focused gates and complete nginx CTest 35/35 pass. Parser/lowering and converter-generated differential remain absent | PARTIAL |
