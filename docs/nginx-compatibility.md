...
| nginx default slash-normalized exact selection for a currently accepted bodyful clean multi-segment key, minimally `location = /a/b { return 200 "ok"; }`, when the raw request target is `/a//b`; #331 | yes: the existing bodyful exact-return model already represents `/a/b`, its borrowed provenance and unchanged 1..64-byte body grammar | yes in `50aed5d7`: every validated bodyful clean exact key now emits exactly one method-omitted ordinary public `route exact slash_normalized` binding with the unchanged strict 200 tuple. Exact goldens/capacity, AST→HIR→MIR→RIR→owned view/lifetime and source validators pass; Raw, duplicate/method/query/Host/nginx-marker/workaround mutations reject | yes: generic checked `SlashNormalized` exact selection from #319 already crosses compiler metadata, owned config and runtime; bodyful trailing-key #320 and bodyless non-trailing #330 prove the primitive, so no new RUT issue is needed | independently approved pinned-nginx oracle `e899a3ca` proves both declaration orders and six vectors: identical exact Date-normalized 144-byte 200/CL2/`ok`/close/EOF, raw-spelling access and live/settled zero upstream for `/a/b`, query and raw `/a//b`; exact 175-byte wires and exactly three ordered byte-exact fallbacks/no fourth for `/a/b/`, `/a/c` and `/`; four reserved ports, isolated resources and fail-closed mutations pass. Converter-generated four-side equivalence is still required | PARTIAL |
