...
| exact `/api/` + clean non-root proxy URI `/v1/`, fresh bodyless explicit-close H1.1 GET domain | no: `ProxyPass.uri` currently admits only `/`; #316 | no; existing `/api/ -> /` lowering must not approximate `/v1/`; #316 | yes: generic bounded clean strip/replace prefix and runtime suffix+query composition already exist | pinned nginx-only oracle `6bd34f28` proves three exact `/v1/` replacement/query upstream histories, two automatic-slash redirects with live/settled zero upstream, and exactly five scoped access records; no converter differential | NOT_IMPLEMENTED |
