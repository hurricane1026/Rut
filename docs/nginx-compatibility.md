...
| the same exact local return for one fresh depth-zero cleartext H1.1 origin-form explicit-close header-absent `DELETE /static` | implicit in the accepted exact model | yes: existing method-omitted exact lowering is sufficient; #296 | no: parser/method key/exact ANY selector represent DELETE, but strict exact-local runtime admission rejects it after selection and closes with zero response; generic capability #295 | committed pinned nginx 1.29.7 baseline under both declaration orders proves exact 200/CL16/full-body/close/EOF and zero upstream; ordinary-RUT production and converter differential pending | BLOCKED_BY_RUT |
