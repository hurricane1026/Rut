...
| `proxy_read_timeout` response-read inactivity timing | yes for explicit raw root-location `1s..63s`; all excluded forms/contexts reject explicitly | no; every present or forged model rejects before RUT emission; #270 lowering blocked by #268 broader method/body domain | partial: ordinary RUT carries exact 1..63s metadata and executes the deadline in narrow cleartext io_uring H1.1 domains: bodyless HEAD with exact positive-header-progress refresh/rearm, plus bodyless GET only when success is exact HTTP/1.1 200 with unique `Content-Length: 0`; every other GET response shape fails closed; nginx location all-method/body semantics remain unavailable and #268 stays open | parser/model/rejection coverage; compiler/config and focused ownership/copy-witness/ledger/arm/refresh/expiry/race/full-timeout-body/GET-retirement regressions; production ordinary-source HEAD zero-byte, fragmented-success, and post-progress-stall wires plus GET zero-byte/full-timeout-body/fresh-origin/CL0-success and fragmented-success wires; GET post-progress-stall evidence is in progress; no converter golden or nginx differential | NOT_IMPLEMENTED |
