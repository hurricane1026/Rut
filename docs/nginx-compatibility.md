...
| nginx default proxied response buffering/commit | implicit | no; #271 blocks faithful lowering; generic lexer-capacity prerequisite #274 is closed | partial: ordinary RUT carries `complete_content_length` through compiler/JIT/config and executes a bounded cleartext io_uring strict-200-positive-CL commit barrier for the bodyless known-non-HEAD exact/ANY domain and, under exactly the fixed HTTP/1.1 strip policy, bounded positive fixed-CL POST/PUT/PATCH uploads on matching exact/ANY routes. Fragmented completion, inactivity, later clean EOF, and initial coalesced complete-response plus same-batch clean-EOF arbitration are implemented. Non-timeout create/connect failures after deadline validation still bypass the configured 502 (#267). HEAD/CONNECT/Other/OPTIONS-star, other request-body methods/framing/capacity, and broader response framing/status/size/transports remain excluded | metadata/forgery/admission plus internal policy-0/policy-1 method/upload/body-wait/quiet-buffering/completion/timeout/EOF/capacity/ownership/race/retirement/D2 tests; the initial coalesced complete+EOF deterministic matrix covers all timeout/CQE orders and persistence modes, and an ordinary policy-1 production wire proves keep-alive then explicit-close over fresh immediately-closing origin episodes; no enabled default-buffering converter golden or nginx-vs-generated-RUT differential | BLOCKED_BY_RUT |
| nginx default buffering for bounded root bodyless origin-form GET, cleartext H1.1, no CL/TE/Expect/Upgrade, strict 200 with one positive bounded CL | implicit | no; the attempted exact-GET lowering in `d5fc3d51` was disabled by `efefc901` after it regressed existing success/gateway behavior | partial: initial same-batch complete+EOF settlement is fixed in `082f20bd` with deterministic and production-wire evidence; configured default-502 selection for validated create/connect failures remains blocked by #267 | rejected draft completion differential exposed both zero-byte paths; rollback restores the committed converter-generated success/gateway suite; the complete+EOF sub-gap now has ordinary-RUT public evidence, while the gateway sub-gap and converter-generated differentials remain | BLOCKED_BY_RUT |
...
- The post-#273 audit identified an exact-HEAD streaming, exact-bodyless-GET
  ...
  (#271), and validated create/connect failures bypass the configured default 502
  (#267). The first gap is now fixed with deterministic and ordinary-RUT public
  evidence; `efefc901` keeps the split disabled until #267 and the converter
