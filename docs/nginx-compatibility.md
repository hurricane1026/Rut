...
| `proxy_read_timeout` response-read inactivity timing | yes for explicit raw root-location `1s..63s`; all excluded forms/contexts reject explicitly | no; every present or forged model rejects before RUT emission; #270 lowering blocked by #268 broader method/body domain | partial: ordinary RUT carries exact 1..63s metadata and, in the cleartext io_uring H1.1 bodyless-HEAD, one-fresh-IPv4 domain, executes the first-response deadline plus exact positive-header-progress refresh/rearm with deterministic batch arbitration; nginx location all-method semantics remain unavailable and #268 stays open | parser/model/rejection coverage; compiler/config and focused ownership/copy-witness/ledger/arm/refresh/expiry/race regressions; production ordinary-source zero-byte, fragmented-success, and post-progress-stall wires with downstream reuse; no converter golden or nginx differential | NOT_IMPLEMENTED |
