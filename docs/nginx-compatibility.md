...
| nginx default proxied response buffering/commit | implicit | no; #271 blocks faithful lowering; generic lexer-capacity prerequisite #274 is closed | partial: ordinary RUT carries `complete_content_length` through compiler/JIT/config and executes a bounded cleartext io_uring strict-200-positive-CL commit barrier for the bodyless known-non-HEAD exact/ANY domain and, under exactly the fixed HTTP/1.1 strip policy, bounded positive fixed-CL POST/PUT/PATCH uploads on matching exact/ANY routes. Fragmented completion, inactivity, and later clean-EOF dispositions are implemented, but an initially coalesced complete fixed-CL response followed by clean EOF in the same batch closes downstream before settlement (#271). Non-timeout create/connect failures after deadline validation also bypass the configured 502 (#267). HEAD/CONNECT/Other/OPTIONS-star, other request-body methods/framing/capacity, and broader response framing/status/size/transports remain excluded | metadata/forgery/admission plus internal policy-0/policy-1 method/upload/body-wait/quiet-buffering/completion/timeout/EOF/capacity/ownership/race/retirement/D2 tests; ordinary-RUT and pinned-nginx disposition wires exist, but the initial coalesced complete+EOF and validated connect-failure regressions remain open; no enabled default-buffering converter golden or nginx-vs-generated-RUT differential | BLOCKED_BY_RUT |
| nginx default buffering for bounded root bodyless origin-form GET, cleartext H1.1, no CL/TE/Expect/Upgrade, strict 200 with one positive bounded CL | implicit | no; the attempted exact-GET lowering in `d5fc3d51` was disabled by `efefc901` after it regressed existing success/gateway behavior | partial: the declared policy-1 domain still lacks initial same-batch complete+EOF settlement (#271) and configured default-502 selection for validated create/connect failures (#267) | rejected draft completion differential exposed the two zero-byte paths; rollback restores the committed converter-generated success/gateway suite; separate pinned-nginx and ordinary-RUT completion/inactivity/EOF evidence remains | BLOCKED_BY_RUT |
...
- The post-#273 audit identified an exact-HEAD streaming, exact-bodyless-GET
  ...
  fixed-CL response plus same-batch clean EOF closes before settlement (#271),
  and validated create/connect failures bypass the configured default 502
  (#267). `efefc901` disables that split and restores the previously proven
  generated source until both gaps and converter differentials land. `/api/`
  remains separately blocked because complete buffering rejects target transforms.
