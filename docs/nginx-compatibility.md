...
| nginx pre-location rejection for one fresh depth-zero cleartext H1.1 origin-form explicit-close header-absent `TRACE /static` with the accepted root proxy plus exact local-return configuration | implicit closed semantic profile in the accepted exact-root model | yes; #302. Emits ordinary concrete `pre_route TRACE` before unmatched/routes; a TRACE-specific route, unmatched fallback, or runtime workaround is forbidden | yes for this bounded generic source/runtime slice; #303 closed. Concrete-only `pre_route METHOD { return local_response(...) }` crosses parser/AST/HIR/MIR/RIR/verifier/printer, transactionally populates RouteConfig, and executes before exact/prefix/unmatched selection in the real io_uring frontend | pinned nginx 1.29.7 baseline under both declaration orders, independent ordinary-RUT source production, and converter golden/owned RouteConfig evidence prove their respective stages; converter-generated nginx-vs-RUT differential remains absent | PARTIAL |
