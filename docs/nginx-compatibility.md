...
| complete existing 2..62-byte clean non-root exact-path class excluding reserved exact `/old`, with literal bodyless `return 204;`, optional meaningful trailing slash, accepted root proxy and the same fresh bodyless explicit-close H1.1 GET domain; #330 | yes structurally: one generic bounded grammar with complete borrowed provenance, `/x`, mixed-alphabet multi-segment and trailing-slash examples, 62/63-byte boundary, empty/dot-segment/dirty/root/variable/control/non-ASCII exclusions and action isolation | yes in `90a62541`: every validated key emits exactly one ordinary public SlashNormalized exact GET with the unchanged strict NoContent204 tuple. Parser/model and RUT runtime are unchanged; exact goldens, 5551..5563 sample sizes, 5619 maximum, Raw-rejection mutations and AST→HIR→MIR→RIR→owned view/source-destruction tests pass. A genuine 62-byte non-trailing key deterministically matches literal/query/embedded-`//` views and misses slash extension/neighbor/root | partial: generic checked SlashNormalized exact selection from #319 and strict NoContent204 from #323 produce the correct wire/route behavior, but #332 is required to preserve the complete checked raw request target in asynchronous access logging beyond 63 bytes. Current `req_path[64]`/`AccessLogEntry::path[64]` silently truncates the query tail | independently approved oracle `b3095fe6`, lowering `90a62541`, focused 5/5 and complete nginx 45/45 pass. The retained four-side harness then proves genuine generated source, 105-byte 204/EOF/zero-upstream and all fallback histories before failing exact access comparison: nginx logs full `<NONTRAILING62>?x=1`, RUT logs only through `?`. #332 blocks final equivalence; no expectation weakening or converter workaround is permitted | BLOCKED_BY_RUT |
