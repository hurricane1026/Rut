...
| nginx default slash-normalized exact selection for a currently accepted bodyful clean multi-segment key, minimally `location = /a/b { return 200 "ok"; }`, when the raw request target is `/a//b`; #331 | yes: the existing bodyful exact-return model already represents `/a/b`, its borrowed provenance and unchanged 1..64-byte body grammar | yes in `50aed5d7`: every validated bodyful clean exact key now emits exactly one method-omitted ordinary public `route exact slash_normalized` binding with the unchanged strict 200 tuple. Exact goldens/capacity, AST→HIR→MIR→RIR→owned view/lifetime and source validators pass; Raw, duplicate/method/query/Host/nginx-marker/workaround mutations reject | partial: generic checked `SlashNormalized` exact selection from #319 is sufficient for the local action, and ordinary non-deadline RUT can forward 201. However #333 is required to preserve the valid 201/CL8 fallback status/reason through the converter-generated nginx-default 60s response-read deadline plus complete-CL buffering path; it currently closes before downstream commit because multiple ownership boundaries hard-require 200 | independently approved pinned-nginx oracle `e899a3ca` proves both declaration orders and six vectors. The first genuine generated public-CLI/io_uring Stage 3 run reaches a live backend for `/a/b/` but receives EOF with zero downstream bytes instead of nginx's exact 175-byte 201/CL8/`fallback` wire, matching the existing RUT fail-closed 201 regression. The retained expectation will not be weakened and the generated timeout will not be removed | BLOCKED_BY_RUT |
