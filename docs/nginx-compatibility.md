...
| nginx pre-location rejection for one fresh depth-zero cleartext H1.1 origin-form explicit-close header-absent `TRACE /static` with the accepted root proxy plus exact local-return configuration | implicit in the accepted exact model | no; #302. A TRACE-specific route, unmatched fallback, or converter workaround would not preserve nginx precedence | yes for this bounded generic source/runtime slice; #303. Concrete-only `pre_route METHOD { return local_response(...) }` crosses parser/AST/HIR/MIR/RIR/verifier/printer, transactionally populates RouteConfig, and executes before exact/prefix/unmatched selection in the real io_uring frontend | pinned nginx 1.29.7 baseline under both declaration orders and an independent ordinary-RUT source production test each prove the fixed ordered 405/CL157/full-nginx-body/close/EOF wire and zero TRACE upstream attempts; no converter-generated differential exists | NOT_IMPLEMENTED |
