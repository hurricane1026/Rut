...
| nginx default proxied response buffering/commit | implicit | no; #271 blocks faithful lowering | partial: ordinary RUT carries `complete_content_length` through compiler/JIT/config and executes an internal bounded cleartext io_uring exact-GET/strict-200-positive-CL commit barrier; full completion, inactivity, and clean-EOF dispositions are implemented, but broader shapes and public equivalence evidence are absent | metadata/forgery/admission plus internal quiet-buffering/completion/timeout/EOF/capacity/ownership/race/retirement tests and manual pinned-nginx 1.29.7 baselines; no ordinary-RUT or converter differential wire | BLOCKED_BY_RUT |
