...
| nginx pre-location rejection for one fresh depth-zero cleartext H1.1 origin-form explicit-close header-absent `TRACE /api/x` with the accepted `/api/` proxy-URI replacement configuration | yes: closed server-level semantic profile; #306 | yes: emits the canonical ordinary concrete `pre_route TRACE` before unmatched/routes while retaining redirect and URI transform | yes: closed generic bounded `pre_route TRACE` capability #303 | pinned nginx-only oracle proves the fixed ordered 405/CL157/full-nginx-body/close/EOF wire and zero upstream; converter golden, target-transform retention and post-lifetime owned RouteConfig evidence pass, but no converter-generated nginx-vs-RUT differential exists | PARTIAL |
