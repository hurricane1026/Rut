...
| the same exact bodyless `return 204;` config for one fresh explicit-close H1.1 literal `GET /static?x=1`; #325 | implicit in the accepted distinct action; request query does not alter nginx config parsing | existing canonical exact GET route is unchanged and contains no query-specific source; generic exact selection treats `?` as the path boundary | yes in no-code probe: current public exact GET NoContent204 route matches the query target through real CLI/io_uring; no new RUT primitive or issue is needed | independently approved required pinned-nginx oracle `bddf01c3` proves both declaration orders over four unique ports: exact 105-byte wire/EOF, live/settled zero upstream, raw query access accounting and one exact fallback episode; Sol new-oracle repeats, #324 regression and complete nginx 39/39 pass. A committed converter-generated four-side differential is still required | PARTIAL |
