...
| one bounded clean exact local-return path `/healthz` beside the accepted root proxy, fresh bodyless explicit-close H1.1 GET domain | no: exact local action is still hard-coded to `/static` | no: exact local lowering is still hard-coded to `/static` | yes: generic exact strict-local binding supports clean paths up to 62 bytes | independently approved pinned-nginx-only oracle `59870bb5` proves both declaration orders and five vectors: exact local `/healthz` plus query with live/settled zero upstream, exact root forwarding for `/healthz/`, `/health`, `/`, complete Date-normalized wires/EOF, three ordered backend histories, and five scoped access records per order; focused 2/2 and full pinned-nginx 29/29 pass | NOT_IMPLEMENTED |
