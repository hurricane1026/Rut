...
| nginx default slash-normalized exact selection for a currently accepted bodyful clean multi-segment key, minimally `location = /a/b { return 200 "ok"; }`, when the raw request target is `/a//b`; #331 closed | yes: the existing bodyful exact-return model already represents `/a/b`, its borrowed provenance and unchanged 1..64-byte body grammar | yes in `50aed5d7`: every validated bodyful clean exact key emits exactly one method-omitted ordinary public `route exact slash_normalized` binding with the unchanged strict 200 tuple. Exact goldens/capacity, AST→HIR→MIR→RIR→owned view/lifetime and source validators pass; Raw, duplicate/method/query/Host/nginx-marker/workaround mutations reject | yes for the declared bounded response profile: generic #333 `846e024d` preserves positive-CL `{200,201}` status/reason through response-read-deadline complete buffering with raw→owned→async ownership, and generic #334 `aab5d066` preserves zero or one validated 1..128-byte upstream Content-Type at its canonical wire position. CL0/streaming 201, status 202+, other forbidden headers, broader framing/protocols and nginx buffering semantics remain fail-closed | independently approved oracle `e899a3ca`, retained contract `d48a11a0`/`dfd0f0a4`, public-loader capabilities `846e024d`/`aab5d066`, and the unchanged final four-side differential prove both declaration orders and all 24 observations across six fresh explicit-close H1.1 GET vectors: exact 144-byte local 200/CL2/`ok` wires with live/settled zero upstream, exact 175-byte 201/Content-Type/CL8/`fallback` wires, three ordered byte-exact backend histories/no retry, raw access targets and clean lifecycles. Sol focused gate passes twice, full Network and Integration pass, and complete pinned-nginx suite is 48/48 | SUPPORTED |
