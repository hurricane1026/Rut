...
| the same bounded exact/root-proxy shape with `return 302 http://redirect.example/new;`, for one fresh explicit-close GET | no: parser currently accepts only the pinned 301 form; #313 is CURRENT | no; #313 must lower only the accepted bounded model | yes for the generic bounded fixed-target policy: closed `{301,302}` validator, frontend/JIT/loader/wire/real-io_uring proof and full acceptance are complete in `427ba484`; legacy request-host and all wider status/tuple shapes are unchanged; #312 closed | committed pinned nginx 1.29.7 oracle `e9d67fff` proves both declaration orders and five vectors: exact 342-byte 302 response/EOF, Host independence/query discard, live/settled zero upstream, exact forward histories and five scoped access records per order; no converter differential | NOT_IMPLEMENTED |
