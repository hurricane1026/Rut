...
| exact bodyless `location = /static { return 204; }` beside the accepted root proxy, fresh bodyless explicit-close H1.1 GET only | yes for the literal bounded action: distinct borrowed `ExactNoContentReturnLocation`, exact `/static`, literal status 204 with complete spans/provenance and explicit exclusions; `a0427b82` | yes: provenance-first complete lexical-shell validation emits one canonical ordinary public `route exact GET "/static"` with the complete strict no-content tuple; dual-order/format full goldens, exact 5539/5547 capacity, forgeries and source-destruction owned RouteConfig pass; `184dbb25` | yes for the bounded target: #323 Stages 1–3b2 (`935255f6`, `920a4747`, `c858ee44`, `4b6a03a7`, `d3abaa6b`) define, propagate, serialize and publicly activate only the complete derived `NoContent204` tuple. Stage 4 `fad6be75` proves an ordinary public source through the real CLI/io_uring frontend after source overwrite: exact 105-byte strict wire/EOF/no tail, separate ordinary-204 CL0 regression, live/settled zero upstream, selector/fallback isolation and exact access response accounting | approved pinned-nginx oracle `97ed1750` proves both declaration orders with the exact Date-normalized 105-byte wire/EOF, live/settled zero upstream, one exact fallback episode and access accounting. Stage 3 normal/sanitizer, focused production/oracle/differential and complete nginx 37/37 pass, but no converter-generated nginx-vs-RUT differential exists | PARTIAL |
