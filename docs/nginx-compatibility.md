...
| one bounded clean proxy location prefix `/service/` + proxy URI `/`, fresh bodyless explicit-close H1.1 GET domain | yes: absolute trailing-slash non-empty ASCII-unreserved segments up to the 63-byte request-observable limit, exact borrowed path/span/provenance and illegal/boundary rejection; `1ba9ac8e` | yes: provenance-first generic path validation and ordinary-RUT route/redirect/strip lowering, full-byte `/service/` golden, exact 3700/5937 maximum capacity and owned RouteConfig; `e7be6890` | yes: generic bounded prefix route and strip/replace target transform already exist | pinned nginx oracle `22bd50e1` and converter-generated public-CLI differential `97727e9d` independently prove three exact `/`, `/x`, `/x?y=1` upstream histories, two query-preserving automatic-slash redirects with live/settled zero upstream, five Date-normalized client wires/EOF, and exactly five scoped access records; focused 3/3 and full pinned-nginx 28/28 pass | SUPPORTED |
