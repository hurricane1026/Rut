...
| nginx default proxied response buffering/commit | implicit | no; #271 blocks faithful lowering; generic lexer-capacity prerequisite #274 is closed | partial: ordinary RUT carries `complete_content_length` through compiler/JIT/config and executes a bounded cleartext io_uring strict-200-positive-CL commit barrier for the bodyless known-non-HEAD exact/ANY domain and, under exactly the fixed HTTP/1.1 strip policy, bounded positive fixed-CL POST/PUT/PATCH uploads on matching exact/ANY routes. The bodyless known-non-HEAD domain also admits one parser-proven OWS-trimmed case-insensitive `Connection: close`, pinned through send/retirement/D2 and closed exactly once after the existing disposition. Full completion, inactivity, and clean-EOF dispositions are implemented; HEAD/CONNECT/Other/OPTIONS-star, other request-body methods/framing/capacity, and broader response framing/status/size/transports remain excluded | metadata/forgery/admission plus internal policy-0/policy-1 method/upload/body-wait/quiet-buffering/completion/timeout/EOF/capacity/ownership/race/retirement/D2 tests; exact-close normalization/rejection, pin identity, zero-progress timeout, completion/EOF, both D2 orders, owner-free close, and successor-suppression tests; policy-0 and policy-1 ordinary-RUT GET fragmented-completion/reuse, inactivity/header-only, and clean-EOF/prefix wires, policy-1 explicit-close GET fragmented-completion/downstream-EOF, incomplete-inactivity/header-only, and premature-clean-EOF/prefix wires, bodyless policy-1 exact-POST fragmented-completion/fresh-origin and ANY-TRACE progress-refresh/inactivity/header-only wires, and policy-1 positive-binary-fixed-upload POST fragmented-completion/fresh-origin, progress-refresh/inactivity/header-only, and premature-EOF/prefix wires, with every policy-1 wire asserting the exact rewritten request; committed pinned-nginx bodyless inactivity, fragmented-completion, premature-EOF, explicit-close fragmented-completion, explicit-close incomplete-inactivity, explicit-close premature-origin-EOF, and positive-binary-POST combined request/response-buffering baselines; no default-buffering converter golden, matching explicit-close converter differential, or positive-body nginx-vs-generated-RUT differential | BLOCKED_BY_RUT |
| nginx default buffering for bounded root bodyless origin-form GET, cleartext H1.1, no CL/TE/Expect/Upgrade, strict 200 with one positive bounded CL | implicit | yes; exact GET direct forward carries request policy 1, `complete_content_length`, nginx default 60s timeout, and strict 502/504 bundles; exact HEAD and ANY remain streaming | yes for the declared bounded policy-1 domain | full converter golden and AST/HIR/MIR/RIR/RouteConfig evidence; separate pinned-nginx and ordinary-RUT completion/inactivity/EOF wires exist, but no converter-generated differential yet | NOT_IMPLEMENTED |
...
- The post-#273 audit found one converter-only root slice that can progress
  ...
  does not regress. The route split and compiler/config golden are now
  implemented; the row remains `NOT_IMPLEMENTED` until a converter-generated
  differential lands. `/api/` remains blocked because complete buffering rejects
  target transforms.
