...
| complete existing 2..62-byte clean non-root exact-path class with literal bodyless `return 204;`, optional meaningful trailing slash, accepted root proxy and the same fresh bodyless explicit-close H1.1 GET domain; #330 | yes structurally: one generic bounded grammar with complete borrowed provenance, `/x`, clean multi-segment and trailing-slash examples, 62/63-byte boundary, dirty/root/variable/control/non-ASCII exclusions and action isolation | yes structurally: one provenance-first validator emits ordinary Raw or SlashNormalized exact GET with the unchanged strict NoContent204 tuple; lower/maximum capacities, full goldens, forgeries and source-destruction owned RouteConfig tests exist | yes for the two generic selector views and strict response primitive through #319/#323; no path-literal-specific runtime mode | class-level behavior claim is under no-code evidence audit. #328 proves one Raw path and #329 one trailing-slash path with real four-side equivalence; parser/lowering boundaries are structural, but the matrix does not yet assume those examples alone prove every accepted path. #330 must identify and close any remaining lower/multi-segment/maximum Raw/SlashNormalized boundary evidence | PARTIAL |
