...
| one bounded clean exact local-return path `/healthz` beside the accepted root proxy, fresh bodyless explicit-close H1.1 GET domain | yes: non-root absolute clean ASCII-unreserved segments through the 62-byte exact-binding limit with optional single trailing slash, exact borrowed path/body/location/response spans and redirect-action isolation; `af7ff351` | yes: generic ordinary-RUT exact binding with provenance-first three-way borrowed-source validation, full dual-order golden, exact 5664-byte capacity and post-lifetime owned `RouteConfig`; `880a729a` | yes: generic exact strict-local binding supports clean paths up to 62 bytes | independently approved pinned-nginx oracle `59870bb5` and converter-generated public-CLI differential `1eeee017` prove both declaration orders and five vectors: exact local `/healthz` plus query with live/settled zero upstream, exact root forwarding for `/healthz/`, `/health`, `/`, complete Date-normalized wires/EOF, three ordered backend histories with no fourth, and five scoped access records per order; Sol focused 3/3 and complete nginx 30/30 pass | SUPPORTED |
| nginx default slash-normalized exact selection for a currently accepted bodyful clean multi-segment key, minimally `location = /a/b { return 200 "ok"; }`, when the raw request target is `/a//b`; #331 | yes: the existing bodyful exact-return model already represents `/a/b`, its borrowed provenance and unchanged 1..64-byte body grammar | no: `put_exact_local_return` still emits Raw for non-trailing keys, so generated RUT falls through to the root proxy for `/a//b`; no fallback or approximation is emitted, and body/action grammar remains separate | yes: generic checked `SlashNormalized` exact selection from #319 already crosses compiler metadata, owned config and runtime; bodyful trailing-key #320 and bodyless non-trailing #330 prove the primitive, so no new RUT issue is indicated | independently approved pinned-nginx oracle `e899a3ca` proves both declaration orders and six vectors: identical exact Date-normalized 144-byte 200/CL2/`ok`/close/EOF, raw-spelling access and live/settled zero upstream for `/a/b`, query and raw `/a//b`; exact 175-byte wires and exactly three ordered byte-exact fallbacks/no fourth for `/a/b/`, `/a/c` and `/`; four reserved ports, isolated resources and fail-closed mutations pass. Generated-RUT differential is still required | NOT_IMPLEMENTED |
| exact local-return path `/health/check/` with nginx slash-normalized selection for raw `/health/check//`, beside the accepted root proxy | yes: the bounded clean exact-path model represents `/health/check/` with borrowed spans/provenance; no broader normalized selection model is claimed | yes: validated trailing-slash paths emit the generic ordinary-RUT `slash_normalized` exact view; non-trailing paths remain Raw byte-identical; dual-order full golden, exact 5681-byte capacity and post-lifetime owned RouteConfig/view matching pass; `2181edf2` | yes for the bounded generic H1 exact strict-local slice; #319 closed. Ordinary `slash_normalized` metadata crosses compiler/owned config and uses a checked full raw-target witness, allocation-free slash-only view, explicit Raw/normalized method precedence and H2 fail-close. Broader normalization/protocols/actions remain excluded | independently approved pinned-nginx oracle `7c29dd53`, ordinary-source RUT proof `3a8c194c`, and converter-generated public-CLI/io_uring differential `7cfd5ca5` prove both declaration orders and five vectors: exact local `/health/check/`, query and raw double slash with live/settled zero upstream; no-redirect raw fallback for `/health/check` and `/`; exact Date-normalized wires/EOF, two ordered histories/no third and five raw access records per side/order. Sol focused 4/4 and two post-#311-fix complete nginx suites 32/32 pass | SUPPORTED |
