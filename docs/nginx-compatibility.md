...
| nginx default proxied response buffering/commit | implicit | no; #271 blocks faithful lowering; generic lexer-capacity prerequisite #274 is closed | partial: ordinary RUT carries `complete_content_length` through compiler/JIT/config and executes a bounded cleartext io_uring strict-200-positive-CL commit barrier for the bodyless known-non-HEAD exact/ANY domain and, under exactly the fixed HTTP/1.1 strip policy, bounded positive fixed-CL POST/PUT/PATCH uploads on matching exact/ANY routes. Fragmented completion, inactivity, later clean EOF, and initial coalesced complete-response plus same-batch clean-EOF arbitration are implemented. Validated local SocketCreate/ConnectSubmit failures now select pinned default 502; asynchronous ConnectCompletion still bypasses it (#267). HEAD/CONNECT/Other/OPTIONS-star, other request-body methods/framing/capacity, and broader response framing/status/size/transports remain excluded | metadata/forgery/admission plus internal policy-0/policy-1 method/upload/body-wait/quiet-buffering/completion/timeout/EOF/capacity/ownership/race/retirement/D2 tests; the initial coalesced complete+EOF deterministic matrix covers all timeout/CQE orders and persistence modes, and an ordinary policy-1 production wire proves keep-alive then explicit-close over fresh immediately-closing origin episodes; pre-CQE failure injection proves pinned 502 across both downstream Recv shapes; no enabled default-buffering converter golden or nginx-vs-generated-RUT differential | BLOCKED_BY_RUT |
| nginx default buffering for bounded root bodyless origin-form GET, cleartext H1.1, no CL/TE/Expect/Upgrade, strict 200 with one positive bounded CL | implicit | no; the attempted exact-GET lowering in `d5fc3d51` was disabled by `efefc901` after it regressed existing success/gateway behavior | partial: initial same-batch complete+EOF settlement is fixed in `082f20bd` with deterministic and production-wire evidence; validated SocketCreate/ConnectSubmit use pinned default 502, while asynchronous ConnectCompletion remains blocked by #267 | rejected draft completion differential exposed both zero-byte paths; rollback restores the committed converter-generated success/gateway suite; the complete+EOF and pre-CQE failure sub-gaps now have focused evidence, while async gateway/public converter evidence remains | BLOCKED_BY_RUT |
...
- The post-#273 audit identified an exact-HEAD streaming, exact-bodyless-GET
  ...
  evidence; pre-CQE local failures are also fixed, while async completion remains.
  `efefc901` keeps the split disabled until #267 and the converter
