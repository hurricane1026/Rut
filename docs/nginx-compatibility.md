...
| `proxy_read_timeout` response-read inactivity timing | no | no | partial: ordinary RUT carries exact 1..63s direct-Forward metadata and fails closed before effects; no deadline arm/refresh/cancel lifecycle yet; #268 | compiler/config/preflight regressions only; no runtime timing or nginx differential | BLOCKED_BY_RUT |
