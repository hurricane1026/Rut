...
| one exact `location = /old { return 301 http://redirect.example/new; }` beside the accepted root proxy, for one bounded fresh explicit-close GET | yes: distinct bounded semantic action with source provenance; `6fa09648` | no: lowering attempt stopped without commit; #309 closed the 653-slot token-capacity gap and `ceb05091` activated the exact conditional compiler/runtime path, but #310 still requires ordinary-source real-io_uring acceptance before lowering resumes | partial composition: fixed redirect is closed/proven by #308, generic 768-slot lexer capacity by #309, metadata foundation by `77847b51`, and strict H1 compiler/runtime activation by `ceb05091`; real-io_uring production proof is still absent, while H2 and broader conditional forwarding remain excluded | pinned nginx oracle `da414a71`, independent fixed-redirect RUT production proof, and compiler/JIT/loader/network-mock evidence exist; #310 production wire, converter golden/ownership, and converter-generated differential remain absent | BLOCKED_BY_RUT |
