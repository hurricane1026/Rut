...
- The post-#273 audit identified an exact-HEAD streaming, exact-bodyless-GET
  ...
  `caa9fce3`, and its converter-generated completion differential is committed.
  The separate sequential gateway differential remains before the bounded row
  can be promoted. Pipelined validated-failure successors remain excluded by
  #276; `/api/` remains separately blocked because complete buffering rejects
  target transforms.
