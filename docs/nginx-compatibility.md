...
| one exact literal `/static` local return for a fresh depth-zero cleartext H1.1 origin-form explicit-close POST with exactly one valid `Content-Length: 0`, no other framing/body/tail | implicit in the accepted exact model | no; converter must remain fail-closed until #291 supplies a generic validated-CL0 strict-local admission primitive | no: current strict exact-local admission rejects every request carrying Content-Length, including valid CL0; #291 | pinned nginx 1.29.7 baseline under both location orders proves the CL0 wire is Date-only-equal to header-absent POST: exact 200/CL16/body/close/EOF and zero upstream effects; no RUT or converter equivalence evidence | BLOCKED_BY_RUT |
...
- Pinned nginx 1.29.7 treats one valid `Content-Length: 0` on the bounded fresh
  ...
  Content-Length. Current RUT preserves the CL0 identity but rejects it from
  strict exact-local admission. #291 tracks the required generic validated-CL0
  primitive; duplicate/conflicting CL, CL+TE, positive bodies, extra bytes,
  Expect/Upgrade, reuse, and pipelines remain excluded.
