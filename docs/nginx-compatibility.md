...
| one bounded clean proxy location prefix `/service/` + proxy URI `/`, fresh bodyless explicit-close H1.1 GET domain | no: parser still accepts only `/` or `/api/` | no: converter still requires `/` or `/api/` and fails closed | yes: generic bounded prefix route and strip/replace target transform already exist | independently approved pinned-nginx-only oracle `22bd50e1` proves `/service/`, `/service/x`, query composition, two query-preserving automatic-slash redirects with live/settled zero upstream, exactly three ordered forward histories, no fourth episode, and five scoped access records; focused 2/2 and full pinned-nginx 27/27 pass | NOT_IMPLEMENTED |
