...
| nginx default proxied response buffering/commit | implicit | no; #271 blocks faithful lowering | partial: ordinary RUT carries `complete_content_length` through compiler/JIT/config and executes a bounded cleartext io_uring exact-GET/strict-200-positive-CL commit barrier; full completion, inactivity, and clean-EOF dispositions are implemented, and the profile now admits omission or the fixed HTTP/1.1 strip request policy, but it still excludes broader method/body/response shapes | metadata/forgery/admission plus internal policy-0/policy-1 quiet-buffering/completion/timeout/EOF/capacity/ownership/race/retirement/D2 tests; policy-0 ordinary-RUT fragmented-completion/reuse, inactivity/header-only, and clean-EOF/prefix wires; policy-1 fragmented-completion/reuse and inactivity/header-only with exact rewritten request wires; committed pinned-nginx inactivity, fragmented-completion, and premature-EOF baselines; no policy-1 EOF wire or converter differential | BLOCKED_BY_RUT |
