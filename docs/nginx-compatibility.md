...
| exact local-return path `/health/check/` with nginx slash-normalized selection for raw `/health/check//`, beside the accepted root proxy | current clean exact-path syntax can represent `/health/check/`; no broader normalized selection model is claimed | no faithful lowering yet: finite raw-path aliases are forbidden and the current Raw exact emission does not preserve the double-slash behavior; #320 | yes for the bounded generic H1 exact strict-local slice; #319 closed. Ordinary `slash_normalized` metadata crosses compiler/owned config and uses a checked full raw-target witness, allocation-free slash-only view, explicit Raw/normalized method precedence and H2 fail-close. Broader normalization/protocols/actions remain excluded | pinned-nginx semantics investigation proves both declaration orders: `/health/check/`, query, and raw double trailing slash are local; `/health/check` and `/` proxy with no automatic-slash redirect. Independently approved ordinary-source public-CLI/io_uring proof `3a8c194c` establishes the generic RUT capability with exact wires/EOF, zero-upstream, raw fallback histories and raw access logs. No converter-generated equivalence exists | NOT_IMPLEMENTED |
