...
| one exact literal `location = /static` returning exact 200 beside the accepted root proxy | yes for a 1..64-byte token-safe quoted ASCII body; #286 | yes: emits method-omitted ordinary RUT exact strict-local route beside unchanged root fallback | yes for the bounded cleartext H1.1 literal exact strict-local domain; #287 and #288 closed | golden plus AST/HIR/MIR/RIR/RouteConfig ownership; pinned nginx exact-location baseline, ordinary exact-route production wires, hand-authored D3, and converter-generated explicit-close GET differential with zero upstream effects; #277 causal coalesced converter differential pending | PARTIAL |
