...
| `proxy_read_timeout` response-read inactivity timing | yes for explicit raw root-location `1s..63s`; all excluded forms/contexts reject explicitly | no; every present or forged model rejects before RUT emission; #270 lowering blocked by #268 broader method/body domain | partial: ordinary RUT carries exact 1..63s metadata and executes the deadline in narrow cleartext io_uring H1.1 domains: bodyless HEAD plus known bodyless origin-form non-HEAD GET/POST/PUT/DELETE/PATCH/OPTIONS/TRACE requests, with exact method ownership and success limited to HTTP/1.1 200 with unique `Content-Length: 0`; TRACE requires an ANY route and every other request/response shape fails closed; nginx request-body/response-body semantics remain unavailable and #268 stays open | parser/model/rejection coverage; compiler/config and focused method/profile/ownership/copy-witness/ledger/arm/refresh/expiry/race/full-timeout-body/retirement regressions; production ordinary-source HEAD and GET zero-byte, fragmented-success, and post-progress-stall wires with exact timeout/success and retirement evidence; POST/TRACE public wires are in progress; no converter golden or nginx differential | NOT_IMPLEMENTED |
