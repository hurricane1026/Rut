...
| one exact `location = /old { return 301 http://redirect.example/new; }` beside the accepted root proxy, for one bounded fresh explicit-close GET | no; #307 | no; current exact action only accepts bounded `/static` return-200 body | partial: #308 metadata/ownership `054d4d23` and fixed-profile source/validator/H1 serializer activation `2d9ec5b3` are approved; loader/JIT/io_uring production proof remains absent | committed pinned nginx 1.29.7 oracle `da414a71` under both declaration orders fixes the 366-byte Date-normalized wire, fixed Location independent of Host, query discard, close/EOF, one access record and live/settled zero upstream; ordinary-source production, golden and converter differential remain absent | BLOCKED_BY_RUT |
