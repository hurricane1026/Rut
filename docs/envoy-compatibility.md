...
| Unmatched route → 404 with empty body, lowercase headers | `local_response` `header_order: "date_server_length"` (`local_reply_envoy_h1`, PR5) is implemented: `date, server, [connection: close,] content-length: 0`, unit/wire- and end-to-end-tested byte for byte against the recorded Envoy oracle for both `OPTIONS *` and authority-form `CONNECT` (the latter's `connection: close` is a known gap: Rut has no CONNECT-specific persistence override, so a keep-alive CONNECT request stays keep-alive; documented below). No pinned-Envoy differential-pair run yet (PR6) | PARTIAL |
