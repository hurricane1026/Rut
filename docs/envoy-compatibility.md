...
| HTTP connection manager: v3 `@type`, non-empty `stat_prefix`, `codec_type: "HTTP1"` required, `generate_request_id: false` required, inline `route_config`, `http_filters` = exactly the router | yes: other network filters, other `@type`, `codec_type` omitted/`AUTO`/`HTTP2`/`HTTP3` (AUTO permits downstream h2c, out of scope), `generate_request_id` omitted or `true`, `rds`, `access_log`, `tracing`, `use_remote_address`, `server_name`, non-router HTTP filters rejected | yes: `kShippedRutCapabilities` is all true (PR5); `codec_type: "HTTP1"` is admitted at parse/convert time, but the generated RUT listener has no protocol control to actually reject an h2c connection preface once installed (`AstListenDecl` has no protocol field, see the "HTTP1-only HCM rejects a client that opens with the h2c connection preface" row below, `BLOCKED_BY_RUT`) — rejecting a downstream h2c upgrade is exactly the behavior `codec_type: "HTTP1"` configures Envoy to have | yes for ordinary HTTP/1 request/response admission and shape | golden + unit wire tests; pinned Envoy v1.39.1 pair differential, CI run `36069445967`, zero skips — but the pair differential sends only ordinary HTTP/1 requests, so it establishes no parity for the h2c-rejection behavior this setting names | PARTIAL: SUPPORTED for ordinary HTTP/1 request/response admission and shape; the `codec_type: "HTTP1"` h2c-rejection behavior stays `BLOCKED_BY_RUT` until the generated RUT can disable h2c (see the Blocked-by-Rut table) |
