...
| Envoy feature | parser | converter | RUT capability | behavior test | status |
| --- | --- | --- | --- | --- | --- |
| `match.path` exact match (bytes 0x21-0x7e excluding `?`/`#`/`%`, max 64 bytes) | yes: modeled as `RouteMatchKind::Path`; both `prefix` and `path` on one match, a `path` not starting with `/`, and reserved/oversized bytes rejected | yes: an exact `path` becomes a conditional `req.pathOnly == "..."` arm in its owning node's chain, or (when its literal text equals a node's own text and no other Envoy route ever resolves it) a companion `route exact "N"` 404 | Rut's `route exact` grammar and `req.pathOnly` comparisons cover this once the six capability rows above are met; golden (c)/(e) and the brute-force equivalence test cover it | none | PARTIAL (golden + equivalence test; pair evidence pending) |
| `match.prefix` ending in `/` (e.g. `"/api/"`), in addition to the catch-all `"/"` | yes: modeled as `RouteMatchKind::Prefix`; validated as `"/"` or starting and ending with `/` | yes: each distinct prefix becomes its own RUT node (`route "N"` / `route HEAD "N"`); golden (a)/(b)/(d) cover declaration-order variants | same as `match.path` above | none | PARTIAL (golden + equivalence test; pair evidence pending) |
...
| Path normalization: `merge_slashes`, percent-decoding, and Rut route-trie segment normalization (`"/api/"` / `"/api//v1"` collapse the same as `"/api"` / `"/api/v1"`) vs. Envoy's literal (unnormalized) string-prefix matching | not modeled: `merge_slashes` is not a recognized field | n/a | Rut's trie normalizes empty path segments when selecting a node; PR 8's `req.pathOnly` arm comparisons do not; neither matches Envoy's default (no normalization) exactly | none | NOT_IMPLEMENTED |
