...
| Upstream response with a duplicate Envoy-inline single-value header -- e.g. two `Content-Type`, `Date`, or `Location` fields -- coalesced by Envoy's `HeaderMapImpl` into one entry (comma-joined) rather than forwarded as two physical lines (custom-inlined single-value headers, `source/common/http/header_map_impl.h`) | yes: milestone bootstrap admission does not depend on per-response header duplication | yes: the emitted route's response_policy has no duplicate-header knob, no capability gate | no, fails closed rather than reproducing the join: the `header_order: "upstream"` (Envoy H1) profile forwards headers verbatim in upstream order instead of rebuilding a HeaderMap, so it cannot comma-join a duplicate; a second `content-type`, `date`, or `location` field now makes `build_upstream_order_response_headers` return false, tripping `reject_strict_response` for a `502 Bad Gateway` before any byte reaches the client (`include/rut/runtime/callbacks_impl.h`, the `content_type_count`/`date_count`/`location_count` guards added for the Codex round-8 review of PR #698; `Content-Length` duplicates were already rejected by the existing `content_length_count != 1` precondition, and a duplicate `Server` is deliberately deduplicated first-wins rather than rejected, per the row above) | unit-tested (`tests/test_network.cc: response_policy.upstream_header_order_rejects_duplicate_inline_headers`); no live pinned-Envoy differential-pair observation of the joined value yet | PARTIAL |
