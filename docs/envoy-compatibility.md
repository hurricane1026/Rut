...
| Hop-by-hop headers removed on the upstream request: `connection`, `keep-alive`, `proxy-connection`, `expect`, `upgrade`, and every header the client's `Connection` value nominates; `te` is kept only when its value is exactly `trailers` (stripped otherwise) | `request_policy.strip_headers` six-name list with `host: "preserve"` (`request_envoy_h1`, PR3); TE-exact-trailers and Connection-token nomination are oracle-driven runtime behavior, not a literal strip-list entry. Two shapes stay outside this profile and fail closed rather than matching Envoy byte-for-byte: a `Connection` token that nominates `content-length` (would desync a persistent upstream's framing), and a body-carrying request with `Expect` (no `100 Continue` interim-response support exists) | PARTIAL |
