...
| A `prefix` / `path` segment beginning with `:` (e.g. `"/:tenant/"`) | yes: `:` is not a reserved byte, so the parser admits it like any other printable-ASCII byte | rejects: a generated RUT node interprets a segment starting with `:` as a route parameter (`include/rut/runtime/route_trie.h`), so `route "/:tenant"` would capture and forward `/anything/x`, which Envoy's literal byte match never does; rejected with `"route match segments beginning with \":\" would become a RUT route parameter, not a literal match; not lowered"` (Codex P1 on PR #695) | n/a | none | BLOCKED_BY_RUT |
