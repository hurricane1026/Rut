...
| Envoy feature | parser | converter | RUT capability | behavior test | status |
| --- | --- | --- | --- | --- | --- |
| `match.path` exact match (bytes 0x21-0x7e excluding `?`/`#`/`%`, max 64 bytes) | yes: modeled as `RouteMatchKind::Path`; both `prefix` and `path` on one match, a `path` not starting with `/`, and reserved/oversized bytes rejected | rejects (multi-route path) or falls through to the single-route BLOCKED rows (single-route path); no `path` lowering exists yet | Rut's route trie needs a segment-exact match for `path`; not yet wired to this frontend | none | NOT_IMPLEMENTED |
| `match.prefix` ending in `/` (e.g. `"/api/"`), in addition to the catch-all `"/"` | yes: modeled as `RouteMatchKind::Prefix`; validated as `"/"` or starting and ending with `/` | same as `path` above | segment-aware prefix routes beyond `"/"` are not yet wired to this frontend | none | NOT_IMPLEMENTED |
