...
| Hop-by-hop headers removed on the upstream request: `connection`, `keep-alive`, `proxy-connection`, `expect`, `upgrade`, and every header the client's `Connection` value nominates; `te` is kept only when its value is exactly `trailers` (stripped otherwise) | `request_policy.strip_headers` six-name list with `host: "preserve"` (`request_envoy_h1`, PR3); TE-exact-trailers and Connection-token nomination are oracle-driven runtime behavior, not a literal strip-list entry | PARTIAL |
