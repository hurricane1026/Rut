...
The exact `return 200` profile accepts 1..4094 raw quoted bytes. The two
quote delimiters must also fit the pinned nginx configuration parser’s 4096-byte
token buffer; the runtime strict local-response body limit is 4096 bytes. Printable ASCII and internal spaces
...
into owned RUT source. The source capacities reserve the full 4030-byte increase
