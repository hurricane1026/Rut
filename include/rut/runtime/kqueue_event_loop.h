...
namespace kqueue_yield {

// Shared max-conn constant: both YieldHeap and KqueueEventLoop derive
...
// when the one-shot yield timer must fire next. Entries are tagged with the
...
struct YieldHeap {
    ...
    // Derived from the shared kqueue_yield::kMaxConns so the heap and
    // KqueueEventLoop always agree on capacity.
...
// KqueueEventLoop — concrete, non-template event loop for kqueue backend.
//
// Kqueue is synchronous: the kernel is done with user buffers when
// recv/send returns. No deferred reclamation, no pending_ops tracking,
// no armed flags, no cancel SQEs.
struct KqueueEventLoop : EventLoopCRTP<KqueueEventLoop> {
    KqueueBackend backend;
    TimerWheel timer;
    kqueue_yield::YieldHeap yield_heap;
    // Absolute deadline currently programmed into the one-shot yield timer.
    // 0 means disarmed. Used to avoid redundant kevent syscalls
...
public:
    static constexpr u32 kMaxConns = kqueue_yield::kMaxConns;
    ...
    // The kqueue loop can synchronously tear down a health-probe Connection
    // (close upstream_fd, EV_DELETE, free the slot), so it issues active
    ...
    void run() {
        ...
        while (is_running() && backend.failure_code() == 0) {
        ...
        // stop()/fatal-backend exits also own live sockets, TLS and pending
        // buffers. Release them before Shard tears down pools and config state.
        force_close_all();
    }
    ...
    void drain(u32 period_secs) {
        ...
        // run()'s drain block, the first time it observes draining_. The timer filter kick
    ...
    // keep a reserve for real client accepts (kqueue frees slots synchronously,
    ...
    void clear_upstream_fd(u32 conn_id) {
        if (conn_id < KqueueBackend::kMaxFdMap) backend.upstream_fd_map[conn_id] = -1;
    }

    // Kqueue-owned upstream episode lifecycle boundary used by connect, reuse,
    ...
    // on EVFILT_WRITE leaves upstream_send_state[conn_id].remaining > 0, and the fresh
    // connect's EVFILT_WRITE would otherwise resume that stale send (or a late
    // UpstreamSend completion would be misread as the connect result). kqueue only;
    ...
    // Returns false (no live idle socket) → caller connects fresh. kqueue is
    // synchronous, so detach/reattach is a plain EV_DELETE/EV_ADD pair.
    bool reuse_idle_upstream(Connection& c, u16 upstream_id, u8 backend_idx) {
        ...
        c.upstream_fd = fd;
        if (c.id < KqueueBackend::kMaxFdMap) backend.upstream_fd_map[c.id] = fd;
        return true;
    }

    // Return conn.upstream_fd to the idle pool for a later request to the same
    // endpoint instead of closing it. Fully detaches the fd from kqueue first
    // (EV_DELETE + clear send state + clear the fd↔conn map) so no stale event
    ...
    // connection. kqueue retains such a send in upstream_send_state with `src`
    ...
    // make a later EVFILT_WRITE on a reused upstream fd ship pending_synth's (by then
    // overwritten) bytes to the next backend. Reset it so the handle_kqueueout guard
    ...
    // until the CQE drains, so this method is kqueue-only (reached via requires-guard).
    void discard_upstream_send(Connection& c) {
        if (c.id < KqueueBackend::kMaxFdMap)
            backend.upstream_send_state[c.id] = {
                nullptr, -1, 0, 0, IoEventType::UpstreamSend, false, 0, 0};
    }

    // Remove the upstream read filter after EOF, preserving any pending write.
    // Descriptor ownership remains with close_conn.
    void ws_unpoll_upstream(Connection& c) {
        if (c.upstream_fd >= 0) backend.quiesce_recv(c.id, /*upstream=*/true, c.upstream_episode);
    }

    // Symmetric to ws_unpoll_upstream for the downstream (client) fd: stop kqueue
    // from redelivering a client half-close (EV_EOF) while a tunnel close is
    ...
    // --- CRTP implementations (no if constexpr — kqueue only) ---
    ...
    // CRTP implementation intentionally replaces the base fallback.
    ...
    // side-effects apply. Removes the probe socket from kqueue, closes it, drops
    ...
    void close_conn_impl(Connection& c) {
        ...
        if (c.pending_handler_fn || c.throttle_paused) {
            if (yield_heap.remove_by_conn(c.id) > 0) rearm_yield_timer();
        }
        if (c.fd >= 0) {
        ...
        if (c.id < KqueueBackend::kMaxFdMap) backend.downstream_fd_map[c.id] = -1;
        // Drop any in-flight partial-send bookkeeping so a reused conn_id+fd
        // cannot resurrect a stale send (see KqueueBackend::clear_send_state).
    ...
    // --- Yield timer (ms precision via one-shot timer filter + min-heap) ---
    ...
    // the one-shot yield timer when the heap's top deadline moves earlier.
    ...
    [[nodiscard]] bool schedule_yield_timer(Connection& conn, u32 ms) {
        timer.remove(&conn);
        // Kqueue is level-triggered: with all callback slots null during
        // yield, an adversarial peer sending bytes while recv_buf is
        // full would keep waking us on EVFILT_READ for no work. Suspend
        // recv interest until the next submit_recv rearms it.
        backend.pause_recv(conn.id);
        u64 now = kqueue_yield::monotonic_ns();
    ...
    // (Re-)arm the one-shot yield timer for the current heap top. Called after
    // every push and after every drain. Uses yield_timer_armed_ns to skip
    // redundant kevent calls.
    void rearm_yield_timer() {
        if (yield_heap.empty()) {
            if (yield_timer_armed_ns != 0) {
                backend.arm_yield_timer(0);
    ...
    void disarm_yield_timer(Connection& conn) {
        ...
        timer.remove(&conn);
        if (yield_heap.remove_by_conn(conn.id) > 0) rearm_yield_timer();
    }
    ...
    [[nodiscard]] bool arm_throttle_timer(Connection& conn, u64 delay_ns) {
        ...
        u64 deadline = kqueue_yield::monotonic_ns() + delay_ns;
        if (yield_heap.push(deadline, conn.handler_gen, conn.id)) {
            rearm_yield_timer();
            return true;
        }
    ...
    void drain_yield_heap() {
        ...
        while (!yield_heap.empty() && yield_heap.top().deadline_ns <= now) {
            ...
            if (c.throttle_paused) {
                throttle_resume<KqueueEventLoop>(this, c);
                continue;
            }
            ...
            resume_jit_handler<KqueueEventLoop>(this, c);
        }
        rearm_yield_timer();
    }
    ...
    void dispatch(const IoEvent& ev) {
        ...
        switch (ev.type) {
            ...
            case IoEventType::HandlerTimer:
                // the yield timer expired; drain all entries at/past the
                // current clock.
                drain_yield_heap();
                break;
            case IoEventType::Timeout: {
                ...
                //
                // If timer filter coalesced enough ticks to cross upstream_timeout, do
                    ...
                    timer.tick([this](Connection* c) {
                        ...
                        if (c->pending_handler_fn &&
                            ...
                            if (c->protocol == ConnProtocol::Http2 && c->h2 != nullptr &&
                                c->h2->async_stream != 0)
                                h2_proxy_fail<KqueueEventLoop>(this, *c, 504);
                            else
                                respond_upstream_timeout<KqueueEventLoop>(this, *c);
                ...
                if (ev.conn_id < kMaxConns) {
                    ...
                    if (conn.on_recv || conn.on_send || conn.on_upstream_recv ||
                        ...
                        if (yield_kind_matches_event(conn.pending_yield_kind, ev.type)) {
                            ...
                            resume_jit_handler<KqueueEventLoop>(this, conn);
                            break;
                        }
...
public:
    void force_close_all() {
        for (u32 i = 0; i < kMaxConns; i++) {
            ...
            // then free_health_probe closes upstream_fd + EV_DELETE + returns
            // the slot). free_health_probe touches only the still-live loop/backend
            // (kqueue set, fd maps, slice pool), all valid here — force_close_all
