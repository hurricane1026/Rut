...
void h2_proxy_finish(Loop* loop,
    ...
    if (mutations_valid && is_head && response_ctx != nullptr &&
        response_ctx->response_body_mutation_set && !status_forbids_content_length &&
        status != 304) {
        char reversed[10];
        u32 value = representation_body_len;
        do {
    ...
    if (mutations_valid) {
        for (u32 i = 0; i < effective_count; i++) {
            ...
            const bool retained_head_content_length =
                is_head && !status_forbids_content_length &&
