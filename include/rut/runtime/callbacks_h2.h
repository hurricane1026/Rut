...
// (forwarded to the h1 upstream, response buffered and re-framed). A connection
// may have one pending-body or async episode, never both: conflicting work is
// refused with 503 while request-body DATA is pending, whereas request frames
// queue while an async stream is parked. Unbuffered JIT forwarding and non-timer
